...
- Canonical routing fields remain exactly `condition_id`, `token_id`, and `outcome`; `token_outcome_pair` remains derived only; the legacy market identifier remains non-routing only.
