...
MODULE_PATH = Path("meg/weather/stage2/supplied_runtime_end_to_end_smoke.py")
TEST_PATH = Path("tests/core/test_weather_supplied_runtime_end_to_end_smoke.py")
...
def _assert_blocked_with_reason(
    record: sees.SuppliedRuntimeEndToEndSmokeRecord,
    reason: str,
) -> None:
    result = sees.validate_supplied_runtime_end_to_end_smoke_record(record)
    assert result.passed is False
    assert result.severity is sees.ValidationSeverity.BLOCKED
    assert reason in result.reasons


def test_enums_are_closed_sets() -> None:
    assert sees.EndToEndSmokeStatus.values() == frozenset(
        {
            "end_to_end_smoke_recorded",
            "end_to_end_smoke_missing",
            "end_to_end_smoke_ambiguous",
            "end_to_end_smoke_unsupported",
            "end_to_end_smoke_unknown",
        }
    )
    assert sees.EndToEndSmokeCompletenessStatus.values() == frozenset(
        {
            "end_to_end_smoke_complete",
            "end_to_end_smoke_incomplete",
            "end_to_end_smoke_ambiguous",
            "end_to_end_smoke_unknown",
        }
    )
    assert sees.OperatorReviewStatus.values() == frozenset(
        {
    ...
    assert sees.RuntimeGateStatus.values() == frozenset(
        {
            "runtime_gate_ready",
    ...
    assert sees.ValidationSeverity.values() == frozenset(
        {"passed", "caution", "failed", "blocked"}
    )


def test_dataclass_construction() -> None:
    record = _valid_end_to_end_smoke(provenance_notes="operator supplied smoke")
    ...
    assert record.end_to_end_smoke_status is sees.EndToEndSmokeStatus.END_TO_END_SMOKE_RECORDED
    assert record.provenance_notes == "operator supplied smoke"


def test_mapping_construction_coerces_string_enums_and_nested_dry_run_report_mapping() -> None:
    record = sees.supplied_runtime_end_to_end_smoke_record_from_mapping(
        {
            "condition_id": "condition-1",
            "token_id": "token-1",
            "outcome": "Yes",
            "supplied_runtime_dry_run_report": asdict(_valid_dry_run_report()),
            "smoke_id": "smoke-2",
            "smoke_summary": "Caller supplied end-to-end smoke summary text.",
            "operator_review_summary": "Operator review is required before any later action.",
            "blocked_reason_summary": "",
            "end_to_end_smoke_status": "end_to_end_smoke_recorded",
            "end_to_end_smoke_completeness_status": "end_to_end_smoke_complete",
            "operator_review_status": "operator_review_required",
            "runtime_gate_status": "runtime_gate_ready",
            "provenance_notes": "smoke note",
        }
    )
    ...
    assert record.end_to_end_smoke_status is sees.EndToEndSmokeStatus.END_TO_END_SMOKE_RECORDED
    assert record.end_to_end_smoke_completeness_status is (
        sees.EndToEndSmokeCompletenessStatus.END_TO_END_SMOKE_COMPLETE
    )
    assert record.operator_review_status is sees.OperatorReviewStatus.OPERATOR_REVIEW_REQUIRED
    assert record.runtime_gate_status is sees.RuntimeGateStatus.RUNTIME_GATE_READY
    assert record.provenance_notes == "smoke note"


def test_minimal_valid_end_to_end_smoke_passes() -> None:
    result = sees.validate_supplied_runtime_end_to_end_smoke_record(
        _valid_end_to_end_smoke()
    )

    assert result.passed is True
    assert result.severity is sees.ValidationSeverity.PASSED
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("smoke_id", "smoke_id is missing"),
        ("smoke_summary", "smoke_summary is missing"),
...
def test_blank_required_text_fields_fail_closed(field_name: str, reason: str) -> None:
    _assert_blocked_with_reason(
        _valid_end_to_end_smoke(**{field_name: "  "}, blocked_reason_summary="blocked"),
        reason,
    )


def test_blank_blocked_reason_summary_is_allowed_for_passing_smoke_record() -> None:
    result = sees.validate_supplied_runtime_end_to_end_smoke_record(
        _valid_end_to_end_smoke(blocked_reason_summary="")
    )
...
def test_blank_blocked_reason_summary_fails_when_smoke_record_is_otherwise_blocked() -> None:
    result = sees.validate_supplied_runtime_end_to_end_smoke_record(
        _valid_end_to_end_smoke(smoke_summary="  ", blocked_reason_summary="")
    )

    assert result.passed is False
    assert result.reasons == (
        "smoke_summary is missing",
        "blocked_reason_summary is missing",
    )


def test_nested_invalid_supplied_runtime_dry_run_report_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_end_to_end_smoke(
            supplied_runtime_dry_run_report=_valid_dry_run_report(report_summary="  "),
            blocked_reason_summary="blocked",
        ),
        "supplied runtime dry-run report validation failed",
    )


def test_condition_id_mismatch_with_supplied_runtime_dry_run_report_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_end_to_end_smoke(
            supplied_runtime_dry_run_report=_valid_dry_run_report(condition_id="condition-2"),
            blocked_reason_summary="blocked",
        ),
        "condition_id does not match supplied runtime dry-run report",
    )


def test_token_id_mismatch_with_supplied_runtime_dry_run_report_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_end_to_end_smoke(
            supplied_runtime_dry_run_report=_valid_dry_run_report(token_id="token-2"),
            blocked_reason_summary="blocked",
        ),
        "token_id does not match supplied runtime dry-run report",
    )


def test_outcome_mismatch_with_supplied_runtime_dry_run_report_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_end_to_end_smoke(
            supplied_runtime_dry_run_report=_valid_dry_run_report(outcome="No"),
            blocked_reason_summary="blocked",
        ),
        "outcome does not match supplied runtime dry-run report",
    )
...
def test_operator_review_summary_mismatch_with_supplied_runtime_dry_run_report_fails_closed() -> None:
    _assert_blocked_with_reason(
        ...
        "operator_review_summary does not match supplied runtime dry-run report",
    )


@pytest.mark.parametrize(
    "end_to_end_smoke_status",
    (
        sees.EndToEndSmokeStatus.END_TO_END_SMOKE_MISSING,
        sees.EndToEndSmokeStatus.END_TO_END_SMOKE_AMBIGUOUS,
        sees.EndToEndSmokeStatus.END_TO_END_SMOKE_UNSUPPORTED,
        sees.EndToEndSmokeStatus.END_TO_END_SMOKE_UNKNOWN,
    ),
)
def test_non_recorded_end_to_end_smoke_statuses_fail_closed(
    end_to_end_smoke_status: sees.EndToEndSmokeStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_end_to_end_smoke(
            end_to_end_smoke_status=end_to_end_smoke_status,
            blocked_reason_summary="blocked",
        ),
        f"end-to-end smoke status is {end_to_end_smoke_status.value}",
    )
...
@pytest.mark.parametrize(
    "end_to_end_smoke_completeness_status",
    (
        sees.EndToEndSmokeCompletenessStatus.END_TO_END_SMOKE_INCOMPLETE,
        sees.EndToEndSmokeCompletenessStatus.END_TO_END_SMOKE_AMBIGUOUS,
        sees.EndToEndSmokeCompletenessStatus.END_TO_END_SMOKE_UNKNOWN,
    ),
)
def test_non_complete_end_to_end_smoke_completeness_statuses_fail_closed(
    end_to_end_smoke_completeness_status: sees.EndToEndSmokeCompletenessStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_end_to_end_smoke(
            end_to_end_smoke_completeness_status=end_to_end_smoke_completeness_status,
            blocked_reason_summary="blocked",
        ),
        f"end-to-end smoke completeness status is {end_to_end_smoke_completeness_status.value}",
    )
...
@pytest.mark.parametrize(
    ...
    (
        sees.OperatorReviewStatus.OPERATOR_REVIEW_MISSING,
        sees.OperatorReviewStatus.OPERATOR_REVIEW_AMBIGUOUS,
        sees.OperatorReviewStatus.OPERATOR_REVIEW_NOT_REQUIRED,
        sees.OperatorReviewStatus.OPERATOR_REVIEW_UNKNOWN,
    ),
)
def test_non_required_operator_review_statuses_fail_closed(
    operator_review_status: sees.OperatorReviewStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_end_to_end_smoke(
...
@pytest.mark.parametrize(
    "runtime_gate_status",
    (
        sees.RuntimeGateStatus.RUNTIME_GATE_BLOCKED,
        sees.RuntimeGateStatus.RUNTIME_GATE_REQUIRES_MANUAL_REVIEW,
        sees.RuntimeGateStatus.RUNTIME_GATE_UNKNOWN,
    ),
)
def test_non_ready_runtime_gate_statuses_fail_closed(
    runtime_gate_status: sees.RuntimeGateStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_end_to_end_smoke(
