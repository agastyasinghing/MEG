...
MODULE_PATH = Path("meg/weather/stage2/supplied_runtime_operator_review_queue_entry.py")
TEST_PATH = Path("tests/core/test_weather_supplied_runtime_operator_review_queue_entry.py")
...
def _assert_blocked_with_reason(
    record: soqe.SuppliedRuntimeOperatorReviewQueueEntryRecord,
    reason: str,
) -> None:
    result = soqe.validate_supplied_runtime_operator_review_queue_entry_record(record)
    assert result.passed is False
    assert result.severity is soqe.ValidationSeverity.BLOCKED
    assert reason in result.reasons


def test_enums_are_closed_sets() -> None:
    assert soqe.OperatorReviewQueueEntryStatus.values() == frozenset(
        {
            "operator_review_queue_entry_recorded",
            "operator_review_queue_entry_missing",
            "operator_review_queue_entry_ambiguous",
            "operator_review_queue_entry_unsupported",
            "operator_review_queue_entry_unknown",
        }
    )
    assert soqe.OperatorReviewQueueEntryCompletenessStatus.values() == frozenset(
        {
            "operator_review_queue_entry_complete",
            "operator_review_queue_entry_incomplete",
            "operator_review_queue_entry_ambiguous",
            "operator_review_queue_entry_unknown",
        }
    )
    assert soqe.OperatorReviewQueueEntryPosture.values() == frozenset(
        {
            "operator_review_queue_entry_in_memory_only",
            "operator_review_queue_entry_missing",
            "operator_review_queue_entry_ambiguous",
            "operator_review_queue_entry_unsupported",
            "operator_review_queue_entry_unknown",
    ...
    assert soqe.OperatorReviewStatus.values() == frozenset(
        {
    ...
    assert soqe.RuntimeGateStatus.values() == frozenset(
        {
            "runtime_gate_ready",
    ...
    assert soqe.ValidationSeverity.values() == frozenset(
        {"passed", "caution", "failed", "blocked"}
    )


def test_dataclass_construction() -> None:
    record = _valid_operator_review_queue_entry(provenance_notes="operator supplied queue")
    ...
    assert record.operator_review_queue_entry_status is (
        soqe.OperatorReviewQueueEntryStatus.OPERATOR_REVIEW_QUEUE_ENTRY_RECORDED
    )
    assert record.provenance_notes == "operator supplied queue"


def test_mapping_construction_coerces_string_enums_and_nested_queue_packet_mapping() -> None:
    record = soqe.supplied_runtime_operator_review_queue_entry_record_from_mapping(
        {
            "condition_id": "condition-1",
            "token_id": "token-1",
            "outcome": "Yes",
            "supplied_runtime_operator_review_queue_packet": asdict(
                _valid_operator_review_queue_packet()
            ),
            "queue_entry_id": "queue-packet-2",
            "queue_entry_summary": "Caller supplied operator-review queue entry summary text.",
            "operator_review_summary": "Operator review is required before any later action.",
            "blocked_reason_summary": "",
            "operator_review_queue_entry_status": "operator_review_queue_entry_recorded",
            "operator_review_queue_entry_completeness_status": "operator_review_queue_entry_complete",
            "operator_review_queue_entry_posture": "operator_review_queue_entry_in_memory_only",
    ...
    assert record.operator_review_queue_entry_completeness_status is (
        soqe.OperatorReviewQueueEntryCompletenessStatus.OPERATOR_REVIEW_QUEUE_ENTRY_COMPLETE
    )
    assert record.operator_review_queue_entry_posture is (
        soqe.OperatorReviewQueueEntryPosture.OPERATOR_REVIEW_QUEUE_ENTRY_IN_MEMORY_ONLY
    )
    assert record.operator_review_status is soqe.OperatorReviewStatus.OPERATOR_REVIEW_REQUIRED
    assert record.runtime_gate_status is soqe.RuntimeGateStatus.RUNTIME_GATE_READY
    assert record.provenance_notes == "queue note"


def test_minimal_valid_operator_review_queue_entry_passes() -> None:
    result = soqe.validate_supplied_runtime_operator_review_queue_entry_record(
        _valid_operator_review_queue_entry()
    )

    assert result.passed is True
    assert result.severity is soqe.ValidationSeverity.PASSED
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("queue_entry_id", "queue_entry_id is missing"),
        ("queue_entry_summary", "queue_entry_summary is missing"),
...
def test_blank_blocked_reason_summary_is_allowed_for_passing_queue_entry() -> None:
    result = soqe.validate_supplied_runtime_operator_review_queue_entry_record(
        _valid_operator_review_queue_entry(blocked_reason_summary="")
    )
...
def test_blank_blocked_reason_summary_fails_when_queue_entry_is_otherwise_blocked() -> None:
    result = soqe.validate_supplied_runtime_operator_review_queue_entry_record(
        _valid_operator_review_queue_entry(queue_entry_summary="  ", blocked_reason_summary="")
    )

    assert result.passed is False
    assert result.reasons == (
        "queue_entry_summary is missing",
        "blocked_reason_summary is missing",
    )


def test_nested_invalid_supplied_runtime_operator_review_queue_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_entry(
            supplied_runtime_operator_review_queue_packet=_valid_operator_review_queue_packet(
                queue_summary="  "
        ...
        "supplied runtime operator-review queue packet validation failed",
    )


def test_condition_id_mismatch_with_supplied_runtime_operator_review_queue_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_entry(
            supplied_runtime_operator_review_queue_packet=_valid_operator_review_queue_packet(
        ...
        "condition_id does not match supplied runtime operator-review queue packet",
    )


def test_token_id_mismatch_with_supplied_runtime_operator_review_queue_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_entry(
            supplied_runtime_operator_review_queue_packet=_valid_operator_review_queue_packet(
                token_id="token-2"
        ...
        "token_id does not match supplied runtime operator-review queue packet",
    )


def test_outcome_mismatch_with_supplied_runtime_operator_review_queue_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_entry(
            supplied_runtime_operator_review_queue_packet=_valid_operator_review_queue_packet(
                outcome="No"
        ...
        "outcome does not match supplied runtime operator-review queue packet",
    )
...
@pytest.mark.parametrize(
    "operator_review_queue_entry_status",
    (
        soqe.OperatorReviewQueueEntryStatus.OPERATOR_REVIEW_QUEUE_ENTRY_MISSING,
        soqe.OperatorReviewQueueEntryStatus.OPERATOR_REVIEW_QUEUE_ENTRY_AMBIGUOUS,
        soqe.OperatorReviewQueueEntryStatus.OPERATOR_REVIEW_QUEUE_ENTRY_UNSUPPORTED,
        soqe.OperatorReviewQueueEntryStatus.OPERATOR_REVIEW_QUEUE_ENTRY_UNKNOWN,
    ),
)
def test_non_recorded_operator_review_queue_entry_statuses_fail_closed(
    operator_review_queue_entry_status: soqe.OperatorReviewQueueEntryStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_entry(
            operator_review_queue_entry_status=operator_review_queue_entry_status,
            blocked_reason_summary="blocked",
        ),
        f"operator review queue entry status is {operator_review_queue_entry_status.value}",
    )
...
@pytest.mark.parametrize(
    "operator_review_queue_entry_completeness_status",
    (
        soqe.OperatorReviewQueueEntryCompletenessStatus.OPERATOR_REVIEW_QUEUE_ENTRY_INCOMPLETE,
        soqe.OperatorReviewQueueEntryCompletenessStatus.OPERATOR_REVIEW_QUEUE_ENTRY_AMBIGUOUS,
        soqe.OperatorReviewQueueEntryCompletenessStatus.OPERATOR_REVIEW_QUEUE_ENTRY_UNKNOWN,
    ),
)
def test_non_complete_operator_review_queue_entry_completeness_statuses_fail_closed(
    operator_review_queue_entry_completeness_status: soqe.OperatorReviewQueueEntryCompletenessStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_entry(
            operator_review_queue_entry_completeness_status=operator_review_queue_entry_completeness_status,
            blocked_reason_summary="blocked",
        ),
        "operator review queue entry completeness status is "
        f"{operator_review_queue_entry_completeness_status.value}",
    )
...
@pytest.mark.parametrize(
    "operator_review_queue_entry_posture",
    (
        soqe.OperatorReviewQueueEntryPosture.OPERATOR_REVIEW_QUEUE_ENTRY_MISSING,
        soqe.OperatorReviewQueueEntryPosture.OPERATOR_REVIEW_QUEUE_ENTRY_AMBIGUOUS,
        soqe.OperatorReviewQueueEntryPosture.OPERATOR_REVIEW_QUEUE_ENTRY_UNSUPPORTED,
        soqe.OperatorReviewQueueEntryPosture.OPERATOR_REVIEW_QUEUE_ENTRY_UNKNOWN,
    ),
)
def test_non_in_memory_only_operator_review_queue_entry_postures_fail_closed(
    operator_review_queue_entry_posture: soqe.OperatorReviewQueueEntryPosture,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_entry(
            operator_review_queue_entry_posture=operator_review_queue_entry_posture,
            blocked_reason_summary="blocked",
        ),
        f"operator review queue entry posture is {operator_review_queue_entry_posture.value}",
    )
...
@pytest.mark.parametrize(
    ...
    (
        soqe.OperatorReviewStatus.OPERATOR_REVIEW_MISSING,
        soqe.OperatorReviewStatus.OPERATOR_REVIEW_AMBIGUOUS,
        soqe.OperatorReviewStatus.OPERATOR_REVIEW_NOT_REQUIRED,
        soqe.OperatorReviewStatus.OPERATOR_REVIEW_UNKNOWN,
    ),
)
def test_non_required_operator_review_statuses_fail_closed(
    operator_review_status: soqe.OperatorReviewStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_entry(
...
@pytest.mark.parametrize(
    "runtime_gate_status",
    (
        soqe.RuntimeGateStatus.RUNTIME_GATE_BLOCKED,
        soqe.RuntimeGateStatus.RUNTIME_GATE_REQUIRES_MANUAL_REVIEW,
        soqe.RuntimeGateStatus.RUNTIME_GATE_UNKNOWN,
    ),
)
def test_non_ready_runtime_gate_statuses_fail_closed(
    runtime_gate_status: soqe.RuntimeGateStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_entry(
