...
EXPECTED_PREDECESSOR_SECTION = """Immediate predecessor: pr_364.
...
EXPECTED_ACTUAL_MERGE_SENTENCE = "Verified actual PR #364 merge commit: cd3d2c5eff10e9a86b8548b3b49912962a52c3c6."
FORBIDDEN_PREVIEW_AS_ACTUAL_SENTENCE = "Verified actual PR #364 merge commit: d81c597bdf93bc7ade4efbaa1110c9e65f51cb61."
EXPECTED_PREVIEW_NEGATIVE_SENTENCE = "The forbidden preview merge SHA d81c597bdf93bc7ade4efbaa1110c9e65f51cb61 is not used as the actual merge commit. The current planning scope is based on repository history containing the actual PR #364 merge commit, and no newer controlling Weather Bot artifact in the Stage 3 contract-planning chain supersedes PR #364 for this evidence-gate decision-record scope."
...
def _validate(text: str | None = None) -> None:
    ...
    assert predecessor == EXPECTED_PREDECESSOR_SECTION
    assert doc.count(EXPECTED_ACTUAL_MERGE_SENTENCE) == 1
    assert FORBIDDEN_PREVIEW_AS_ACTUAL_SENTENCE not in doc
    assert doc.count(EXPECTED_PREVIEW_NEGATIVE_SENTENCE) == 1
...
def test_in_memory_mutations_are_rejected() -> None:
    ...
    mutations = [
        ...
        _replace_once(doc, ACTUAL_PR_364_MERGE_SHA, PREVIEW_MERGE_SHA),
        _replace_once(
            doc,
            EXPECTED_ACTUAL_MERGE_SENTENCE,
            f"{EXPECTED_ACTUAL_MERGE_SENTENCE}\n\n{FORBIDDEN_PREVIEW_AS_ACTUAL_SENTENCE}",
        ),
        _replace_once(doc, EXPECTED_ACTUAL_MERGE_SENTENCE, f"{EXPECTED_ACTUAL_MERGE_SENTENCE}\n\n{EXPECTED_ACTUAL_MERGE_SENTENCE}"),
        _replace_once(doc, "is not used as the actual merge commit", "is used as the actual merge commit"),
    ]
