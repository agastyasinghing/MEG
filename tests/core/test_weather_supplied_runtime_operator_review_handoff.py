...
MODULE_PATH = Path("meg/weather/stage2/supplied_runtime_operator_review_handoff.py")
TEST_PATH = Path("tests/core/test_weather_supplied_runtime_operator_review_handoff.py")
...
def _assert_blocked_with_reason(
    record: sroh.SuppliedRuntimeOperatorReviewHandoffRecord,
    reason: str,
) -> None:
    result = sroh.validate_supplied_runtime_operator_review_handoff_record(record)
    assert result.passed is False
    assert result.severity is sroh.ValidationSeverity.BLOCKED
    assert reason in result.reasons


def test_enums_are_closed_sets() -> None:
    assert sroh.OperatorReviewHandoffStatus.values() == frozenset(
        {
            "operator_review_handoff_recorded",
            "operator_review_handoff_missing",
            "operator_review_handoff_ambiguous",
            "operator_review_handoff_unsupported",
            "operator_review_handoff_unknown",
        }
    )
    assert sroh.OperatorReviewHandoffCompletenessStatus.values() == frozenset(
        {
            "operator_review_handoff_complete",
            "operator_review_handoff_incomplete",
            "operator_review_handoff_ambiguous",
            "operator_review_handoff_unknown",
        }
    )
    assert sroh.OperatorReviewStatus.values() == frozenset(
        {
    ...
    assert sroh.RuntimeGateStatus.values() == frozenset(
        {
            "runtime_gate_ready",
    ...
    assert sroh.ValidationSeverity.values() == frozenset(
        {"passed", "caution", "failed", "blocked"}
    )


def test_dataclass_construction() -> None:
    record = _valid_operator_review_handoff(provenance_notes="operator supplied handoff")

    assert record.condition_id == "condition-1"
    assert record.token_id == "token-1"
    assert record.outcome == "Yes"
    assert isinstance(
        record.supplied_runtime_trace_packet,
        srtp.SuppliedRuntimeTracePacketRecord,
    )
    assert record.operator_review_handoff_status is (
        sroh.OperatorReviewHandoffStatus.OPERATOR_REVIEW_HANDOFF_RECORDED
    )
    assert record.provenance_notes == "operator supplied handoff"


def test_mapping_construction_coerces_string_enums_and_nested_trace_packet_mapping() -> None:
    record = sroh.supplied_runtime_operator_review_handoff_record_from_mapping(
        {
            "condition_id": "condition-1",
            "token_id": "token-1",
            "outcome": "Yes",
            "supplied_runtime_trace_packet": asdict(_valid_trace_packet()),
            "handoff_id": "handoff-2",
            "handoff_summary": "Caller supplied operator-review handoff summary text.",
            "operator_review_summary": "Operator review is required before any later action.",
            "blocked_reason_summary": "",
            "operator_review_handoff_status": "operator_review_handoff_recorded",
            "operator_review_handoff_completeness_status": "operator_review_handoff_complete",
            "operator_review_status": "operator_review_required",
            "runtime_gate_status": "runtime_gate_ready",
            "provenance_notes": "handoff note",
        }
    )
    ...
    assert record.operator_review_handoff_completeness_status is (
        sroh.OperatorReviewHandoffCompletenessStatus.OPERATOR_REVIEW_HANDOFF_COMPLETE
    )
    assert record.operator_review_status is sroh.OperatorReviewStatus.OPERATOR_REVIEW_REQUIRED
    assert record.runtime_gate_status is sroh.RuntimeGateStatus.RUNTIME_GATE_READY
    assert record.provenance_notes == "handoff note"


def test_minimal_valid_operator_review_handoff_passes() -> None:
    result = sroh.validate_supplied_runtime_operator_review_handoff_record(
        _valid_operator_review_handoff()
    )

    assert result.passed is True
    assert result.severity is sroh.ValidationSeverity.PASSED
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("handoff_id", "handoff_id is missing"),
        ("handoff_summary", "handoff_summary is missing"),
...
def test_blank_required_text_fields_fail_closed(field_name: str, reason: str) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_handoff(**{field_name: "  "}, blocked_reason_summary="blocked"),
        reason,
    )


def test_blank_blocked_reason_summary_is_allowed_for_passing_handoff() -> None:
    result = sroh.validate_supplied_runtime_operator_review_handoff_record(
        _valid_operator_review_handoff(blocked_reason_summary="")
    )
...
def test_blank_blocked_reason_summary_fails_when_handoff_is_otherwise_blocked() -> None:
    result = sroh.validate_supplied_runtime_operator_review_handoff_record(
        _valid_operator_review_handoff(handoff_summary="  ", blocked_reason_summary="")
    )

    assert result.passed is False
    assert result.reasons == (
        "handoff_summary is missing",
        "blocked_reason_summary is missing",
    )


def test_nested_invalid_supplied_runtime_trace_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_handoff(
            supplied_runtime_trace_packet=_valid_trace_packet(trace_summary="  "),
            blocked_reason_summary="blocked",
        ),
        "supplied runtime trace packet validation failed",
    )


def test_condition_id_mismatch_with_supplied_runtime_trace_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_handoff(
            supplied_runtime_trace_packet=_valid_trace_packet(condition_id="condition-2"),
            blocked_reason_summary="blocked",
        ),
        "condition_id does not match supplied runtime trace packet",
    )


def test_token_id_mismatch_with_supplied_runtime_trace_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_handoff(
            supplied_runtime_trace_packet=_valid_trace_packet(token_id="token-2"),
            blocked_reason_summary="blocked",
        ),
        "token_id does not match supplied runtime trace packet",
    )


def test_outcome_mismatch_with_supplied_runtime_trace_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_handoff(
            supplied_runtime_trace_packet=_valid_trace_packet(outcome="No"),
            blocked_reason_summary="blocked",
        ),
        "outcome does not match supplied runtime trace packet",
    )
...
def test_operator_review_summary_mismatch_with_supplied_runtime_trace_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        ...
        "operator_review_summary does not match supplied runtime trace packet",
    )
...
@pytest.mark.parametrize(
    "operator_review_handoff_status",
    (
        sroh.OperatorReviewHandoffStatus.OPERATOR_REVIEW_HANDOFF_MISSING,
        sroh.OperatorReviewHandoffStatus.OPERATOR_REVIEW_HANDOFF_AMBIGUOUS,
        sroh.OperatorReviewHandoffStatus.OPERATOR_REVIEW_HANDOFF_UNSUPPORTED,
        sroh.OperatorReviewHandoffStatus.OPERATOR_REVIEW_HANDOFF_UNKNOWN,
    ),
)
def test_non_recorded_operator_review_handoff_statuses_fail_closed(
    operator_review_handoff_status: sroh.OperatorReviewHandoffStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_handoff(
            operator_review_handoff_status=operator_review_handoff_status,
            blocked_reason_summary="blocked",
        ),
        f"operator review handoff status is {operator_review_handoff_status.value}",
    )
...
@pytest.mark.parametrize(
    "operator_review_handoff_completeness_status",
    (
        sroh.OperatorReviewHandoffCompletenessStatus.OPERATOR_REVIEW_HANDOFF_INCOMPLETE,
        sroh.OperatorReviewHandoffCompletenessStatus.OPERATOR_REVIEW_HANDOFF_AMBIGUOUS,
        sroh.OperatorReviewHandoffCompletenessStatus.OPERATOR_REVIEW_HANDOFF_UNKNOWN,
    ),
)
def test_non_complete_operator_review_handoff_completeness_statuses_fail_closed(
    operator_review_handoff_completeness_status: sroh.OperatorReviewHandoffCompletenessStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_handoff(
            operator_review_handoff_completeness_status=(
        ...
        "operator review handoff completeness status is "
        f"{operator_review_handoff_completeness_status.value}",
    )
...
@pytest.mark.parametrize(
    ...
    (
        sroh.OperatorReviewStatus.OPERATOR_REVIEW_MISSING,
        sroh.OperatorReviewStatus.OPERATOR_REVIEW_AMBIGUOUS,
        sroh.OperatorReviewStatus.OPERATOR_REVIEW_NOT_REQUIRED,
        sroh.OperatorReviewStatus.OPERATOR_REVIEW_UNKNOWN,
    ),
)
def test_non_required_operator_review_statuses_fail_closed(
    operator_review_status: sroh.OperatorReviewStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_handoff(
...
@pytest.mark.parametrize(
    "runtime_gate_status",
    (
        sroh.RuntimeGateStatus.RUNTIME_GATE_BLOCKED,
        sroh.RuntimeGateStatus.RUNTIME_GATE_REQUIRES_MANUAL_REVIEW,
        sroh.RuntimeGateStatus.RUNTIME_GATE_UNKNOWN,
    ),
)
def test_non_ready_runtime_gate_statuses_fail_closed(
    runtime_gate_status: sroh.RuntimeGateStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_handoff(
