"""Static checks for Weather Bot Phase 0A no-lookahead policy documentation."""
...
CANONICAL_ID = "WEATHER-BOT-PHASE0A-NO-LOOKAHEAD-POLICY-DOCUMENTATION-01"
ARTIFACT_PATH = REPO_ROOT / f"docs/prd/{CANONICAL_ID}.md"
TEST_PATH = REPO_ROOT / "tests/core/test_weather_bot_phase0a_no_lookahead_policy_documentation_01.py"
MACHINE_HEADING = "Machine-checkable Weather Bot Phase 0A no-lookahead policy-documentation assignments"
NEXT_TRACK = "weather_bot_phase0a_fail_closed_error_taxonomy_planning"
CONDITIONAL_TRACK = "weather_bot_phase0a_no_lookahead_policy_revision_if_scope_too_broad"

REQUIRED_SECTIONS = (
    "Status and scope", "Relationship to manual-review checklist planning", "Policy objective",
    "Current held/closed source-fetching posture", "No owner-decision revision boundary",
    "No-lookahead policy overview", "Decision-time boundary", "Evidence timestamp boundary",
    "Resolution timestamp boundary", "Forecast and observation boundary", "Settlement-source boundary",
    "Manual-review no-lookahead checklist", "Backtesting and evaluation boundary",
    "Paper-trade readiness boundary", "Static documentation only boundary", "Canonical identifier posture",
    "Source-fetching track remains blocked", "Provider/source execution boundary", "Credential/config boundary",
    "Generated-data and fixture boundary", "Scoring/backtesting boundary", "Trading/autonomy/production boundary",
...
NO_LOOKAHEAD_POLICY_FIELDS = {'operator_review_time', 'forecast_issue_time', 'market_close_time', 'lookahead_violation_detected', 'evidence_observed_time', 'decision_time', 'market_resolution_source_time', 'lookahead_status_unknown_requires_review', 'resolution_time', 'latest_allowed_information_time', 'observation_valid_time', 'evidence_available_time', 'settlement_time'}
NO_LOOKAHEAD_RULES = {'manual_review_if_timestamp_missing', 'no_post_close_market_data', 'no_future_observation_valid_time', 'no_settlement_result_leakage', 'no_post_decision_evidence', 'no_future_forecast_issue_time', 'fail_closed_on_lookahead_uncertainty', 'no_post_resolution_source_data', 'manual_review_if_timestamp_ambiguous', 'manual_review_if_source_time_conflicts'}
MANUAL_REVIEW_NO_LOOKAHEAD_CHECKLIST = {'check_source_timestamp_unambiguous', 'check_observation_valid_time_allowed', 'check_forecast_issue_time_not_future', 'check_evidence_available_time_present', 'check_decision_time_present', 'check_market_close_boundary_respected', 'check_resolution_time_not_used_pre_decision', 'check_settlement_time_not_used_pre_decision', 'check_lookahead_uncertainty_fails_closed', 'check_evidence_not_after_decision_time'}
BLOCKED_WORK = {'owner_decision_revision', 'api_call_execution', 'order_placement', 'production_behavior', 'audit_output_persistence', 'source_fetching_runtime_implementation_plan', 'backtesting_implementation', 'no_lookahead_runtime_enforcement', 'scraping_execution', 'scoring_implementation', 'autonomy_behavior', 'provider_connector_implementation', 'forecast_pull_execution', 'audit_report_generation', 'timestamp_runtime_validation', 'generated_data_creation', 'settlement_rule_runtime_classification', 'manual_review_persistence', 'standalone_self_review_prd_artifact', 'operator_decision_execution', 'manual_review_ui', 'provider_client_creation', 'settlement_rule_runtime_parser', 'paper_trade_execution', 'manual_review_runtime_workflow', 'external_export_behavior', 'source_fetching_implementation', 'fixture_data_modification', 'live_provider_source_fetching', 'provider_sdk_execution', 'runtime_trading_behavior', 'file_download_execution', 'credentials_config_loading'}
STAGE2_PATHS = {
...
ALLOWED_ASSIGNMENTS = {
    ...
    "no lookahead policy status": {"docs_static_test_only", "no_lookahead_policy_documentation_only", "post_weather_bot_phase0a_manual_review_checklist_planning"},
    ...
    "no lookahead policy field": NO_LOOKAHEAD_POLICY_FIELDS,
    "no lookahead rule": NO_LOOKAHEAD_RULES,
    "manual review no lookahead checklist item": MANUAL_REVIEW_NO_LOOKAHEAD_CHECKLIST,
    ...
    "implementation posture": {"docs_static_test_only", "no_lookahead_policy_documentation_only", "no_runtime_code_change", "no_owner_decision_revision", "no_source_fetching", "no_source_fetching_plan", "no_provider_connector", "no_provider_client", "no_live_provider_fetching", "no_credential_config_loading", "no_generated_data", "no_fixture_change", "no_no_lookahead_runtime_enforcement", "no_timestamp_runtime_validation", "no_settlement_rule_runtime_parser", "no_manual_review_runtime_workflow", "no_scoring_backtesting", "no_paper_trade_execution", "no_trading_autonomy_production", "no_report_writing", "no_external_export", "no_persistence"},
...
FORBIDDEN_APPROVAL_RE = re.compile(
    ...
    r"fixture change " "is approved|no-lookahead enforcement " "is approved|no lookahead enforcement " "is approved|"
...
def test_document_exists_canonical_id_and_sections_are_non_empty() -> None:
    ...
    text = _read()
    assert text.startswith(f"# {CANONICAL_ID} — Weather Bot Phase 0A No-Lookahead Policy Documentation")
...
def test_required_posture_and_non_execution_boundaries_are_present() -> None:
    text = _read()
    required = [
        "docs/static-test-only/no-lookahead-policy-documentation-only",
        ...
        "does not implement runtime no-lookahead enforcement",
        ...
        "does not implement scoring, backtesting, trading, or autonomy",
...
def test_policy_values_rules_checklist_blocked_work_and_stage2_paths_appear() -> None:
    text = _read()
    for value in NO_LOOKAHEAD_POLICY_FIELDS | NO_LOOKAHEAD_RULES | MANUAL_REVIEW_NO_LOOKAHEAD_CHECKLIST | BLOCKED_WORK | STAGE2_PATHS:
...
def test_machine_assignments_are_section_scoped_complete_and_allowed() -> None:
    ...
    assert by_field["conditional next track"] == {"weather_bot_phase0a_no_lookahead_policy_revision_if_scope_too_broad"}
