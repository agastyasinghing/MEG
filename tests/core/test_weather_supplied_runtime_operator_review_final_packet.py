...
MODULE_PATH = Path("meg/weather/stage2/supplied_runtime_operator_review_final_packet.py")
TEST_PATH = Path("tests/core/test_weather_supplied_runtime_operator_review_final_packet.py")
...
def _assert_blocked_with_reason(
    record: sofp.SuppliedRuntimeOperatorReviewFinalPacketRecord,
    reason: str,
) -> None:
    result = sofp.validate_supplied_runtime_operator_review_final_packet_record(record)
    assert result.passed is False
    assert result.severity is sofp.ValidationSeverity.BLOCKED
    assert reason in result.reasons


def test_enums_are_closed_sets() -> None:
    assert sofp.OperatorReviewFinalPacketStatus.values() == frozenset(
        {
            "operator_review_final_packet_recorded",
            "operator_review_final_packet_missing",
            "operator_review_final_packet_ambiguous",
            "operator_review_final_packet_unsupported",
            "operator_review_final_packet_unknown",
        }
    )
    assert sofp.OperatorReviewFinalCompletenessStatus.values() == frozenset(
        {
            "operator_review_final_complete",
            "operator_review_final_incomplete",
            "operator_review_final_ambiguous",
            "operator_review_final_unknown",
        }
    )
    assert sofp.OperatorReviewFinalPosture.values() == frozenset(
        {
            "operator_review_final_in_memory_only",
            "operator_review_final_missing",
            "operator_review_final_ambiguous",
            "operator_review_final_unsupported",
            "operator_review_final_unknown",
        }
    )
    assert sofp.OperatorReviewStatus.values() == frozenset(
        {
    ...
    assert sofp.RuntimeGateStatus.values() == frozenset(
        {
            "runtime_gate_ready",
    ...
    assert sofp.ValidationSeverity.values() == frozenset(
        {"passed", "caution", "failed", "blocked"}
    )


def test_dataclass_construction() -> None:
    record = _valid_operator_review_final_packet(provenance_notes="operator supplied final")
    ...
    assert record.operator_review_final_packet_status is (
        sofp.OperatorReviewFinalPacketStatus.OPERATOR_REVIEW_FINAL_PACKET_RECORDED
    )
    assert record.provenance_notes == "operator supplied final"


def test_mapping_construction_coerces_string_enums_and_nested_operator_review_queue_summary_mapping() -> None:
    record = sofp.supplied_runtime_operator_review_final_packet_record_from_mapping(
        {
            "condition_id": "condition-1",
            "token_id": "token-1",
            "outcome": "Yes",
            "supplied_runtime_operator_review_queue_summary": asdict(
                _valid_operator_review_queue_summary()
            ),
            "final_packet_id": "final-packet-2",
            "final_packet_summary": "Caller supplied operator-review final packet summary text.",
            "operator_review_summary": "Operator review is required before any later action.",
            "blocked_reason_summary": "",
            "operator_review_final_packet_status": "operator_review_final_packet_recorded",
            "operator_review_final_completeness_status": "operator_review_final_complete",
            "operator_review_final_posture": "operator_review_final_in_memory_only",
            "operator_review_status": "operator_review_required",
            "runtime_gate_status": "runtime_gate_ready",
            "provenance_notes": "final note",
        }
    )
    ...
    assert record.operator_review_final_completeness_status is (
        sofp.OperatorReviewFinalCompletenessStatus.OPERATOR_REVIEW_FINAL_COMPLETE
    )
    assert record.operator_review_final_posture is (
        sofp.OperatorReviewFinalPosture.OPERATOR_REVIEW_FINAL_IN_MEMORY_ONLY
    )
    assert record.operator_review_status is sofp.OperatorReviewStatus.OPERATOR_REVIEW_REQUIRED
    assert record.runtime_gate_status is sofp.RuntimeGateStatus.RUNTIME_GATE_READY
    assert record.provenance_notes == "final note"


def test_minimal_valid_operator_review_final_packet_passes() -> None:
    result = sofp.validate_supplied_runtime_operator_review_final_packet_record(
        _valid_operator_review_final_packet()
    )

    assert result.passed is True
    assert result.severity is sofp.ValidationSeverity.PASSED
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("final_packet_id", "final_packet_id is missing"),
        ("final_packet_summary", "final_packet_summary is missing"),
...
def test_blank_blocked_reason_summary_is_allowed_for_passing_final_packet() -> None:
    result = sofp.validate_supplied_runtime_operator_review_final_packet_record(
        _valid_operator_review_final_packet(blocked_reason_summary="")
    )
...
def test_blank_blocked_reason_summary_fails_when_final_packet_is_otherwise_blocked() -> None:
    result = sofp.validate_supplied_runtime_operator_review_final_packet_record(
        _valid_operator_review_final_packet(final_packet_summary="  ", blocked_reason_summary="")
    )

    assert result.passed is False
    assert result.reasons == (
        "final_packet_summary is missing",
        "blocked_reason_summary is missing",
    )


def test_nested_invalid_supplied_runtime_operator_review_queue_summary_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_packet(
            supplied_runtime_operator_review_queue_summary=_valid_operator_review_queue_summary(
                queue_summary_text="  "
        ...
        "supplied runtime operator-review queue summary validation failed",
    )


def test_condition_id_mismatch_with_supplied_runtime_operator_review_queue_summary_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_packet(
            supplied_runtime_operator_review_queue_summary=_valid_operator_review_queue_summary(
        ...
        "condition_id does not match supplied runtime operator-review queue summary",
    )


def test_token_id_mismatch_with_supplied_runtime_operator_review_queue_summary_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_packet(
            supplied_runtime_operator_review_queue_summary=_valid_operator_review_queue_summary(
                token_id="token-2"
        ...
        "token_id does not match supplied runtime operator-review queue summary",
    )


def test_outcome_mismatch_with_supplied_runtime_operator_review_queue_summary_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_packet(
            supplied_runtime_operator_review_queue_summary=_valid_operator_review_queue_summary(
                outcome="No"
        ...
        "outcome does not match supplied runtime operator-review queue summary",
    )


def test_operator_review_summary_mismatch_with_supplied_runtime_operator_review_queue_summary_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_packet(
...
@pytest.mark.parametrize(
    "operator_review_final_packet_status",
    (
        sofp.OperatorReviewFinalPacketStatus.OPERATOR_REVIEW_FINAL_PACKET_MISSING,
        sofp.OperatorReviewFinalPacketStatus.OPERATOR_REVIEW_FINAL_PACKET_AMBIGUOUS,
        sofp.OperatorReviewFinalPacketStatus.OPERATOR_REVIEW_FINAL_PACKET_UNSUPPORTED,
        sofp.OperatorReviewFinalPacketStatus.OPERATOR_REVIEW_FINAL_PACKET_UNKNOWN,
    ),
)
def test_non_recorded_operator_review_final_packet_statuses_fail_closed(
    operator_review_final_packet_status: sofp.OperatorReviewFinalPacketStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_packet(
            operator_review_final_packet_status=operator_review_final_packet_status,
            blocked_reason_summary="blocked",
        ),
        f"operator review final packet status is {operator_review_final_packet_status.value}",
    )
...
@pytest.mark.parametrize(
    "operator_review_final_completeness_status",
    (
        sofp.OperatorReviewFinalCompletenessStatus.OPERATOR_REVIEW_FINAL_INCOMPLETE,
        sofp.OperatorReviewFinalCompletenessStatus.OPERATOR_REVIEW_FINAL_AMBIGUOUS,
        sofp.OperatorReviewFinalCompletenessStatus.OPERATOR_REVIEW_FINAL_UNKNOWN,
    ),
)
def test_non_complete_operator_review_final_completeness_statuses_fail_closed(
    operator_review_final_completeness_status: sofp.OperatorReviewFinalCompletenessStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_packet(
            operator_review_final_completeness_status=operator_review_final_completeness_status,
            blocked_reason_summary="blocked",
        ),
        "operator review final completeness status is "
        f"{operator_review_final_completeness_status.value}",
    )
...
@pytest.mark.parametrize(
    "operator_review_final_posture",
    (
        sofp.OperatorReviewFinalPosture.OPERATOR_REVIEW_FINAL_MISSING,
        sofp.OperatorReviewFinalPosture.OPERATOR_REVIEW_FINAL_AMBIGUOUS,
        sofp.OperatorReviewFinalPosture.OPERATOR_REVIEW_FINAL_UNSUPPORTED,
        sofp.OperatorReviewFinalPosture.OPERATOR_REVIEW_FINAL_UNKNOWN,
    ),
)
def test_non_in_memory_only_operator_review_final_postures_fail_closed(
    operator_review_final_posture: sofp.OperatorReviewFinalPosture,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_packet(
            operator_review_final_posture=operator_review_final_posture,
            blocked_reason_summary="blocked",
        ),
        f"operator review final posture is {operator_review_final_posture.value}",
    )
...
@pytest.mark.parametrize(
    ...
    (
        sofp.OperatorReviewStatus.OPERATOR_REVIEW_MISSING,
        sofp.OperatorReviewStatus.OPERATOR_REVIEW_AMBIGUOUS,
        sofp.OperatorReviewStatus.OPERATOR_REVIEW_NOT_REQUIRED,
        sofp.OperatorReviewStatus.OPERATOR_REVIEW_UNKNOWN,
    ),
)
def test_non_required_operator_review_statuses_fail_closed(
    operator_review_status: sofp.OperatorReviewStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_packet(
...
@pytest.mark.parametrize(
    "runtime_gate_status",
    (
        sofp.RuntimeGateStatus.RUNTIME_GATE_BLOCKED,
        sofp.RuntimeGateStatus.RUNTIME_GATE_REQUIRES_MANUAL_REVIEW,
        sofp.RuntimeGateStatus.RUNTIME_GATE_UNKNOWN,
    ),
)
def test_non_ready_runtime_gate_statuses_fail_closed(
    runtime_gate_status: sofp.RuntimeGateStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_packet(
