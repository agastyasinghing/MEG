"""Static checks for Weather Bot Phase 0A Hold State Closeout."""
...
CANONICAL_ID = "WEATHER-BOT-PHASE0A-HOLD-STATE-CLOSEOUT-01"
ARTIFACT_PATH = REPO_ROOT / f"docs/prd/{CANONICAL_ID}.md"
MACHINE_HEADING = "## Machine-checkable Weather Bot Phase 0A hold-state-closeout assignments"
...
ALLOWED_ASSIGNMENTS = {
    ...
    "held state closeout status": {
        "docs_static_test_only",
        "held_state_closeout_only",
        "post_weather_bot_phase0a_hold_state_refresh",
    },
    "closed phase0a posture": {
    ...
    "closed blocked work": BLOCKED_WORK,
    ...
    "conditional next track": {
        "weather_bot_phase0a_hold_state_closeout_revision_if_scope_too_broad",
...
def test_document_exists_canonical_id_and_sections_are_non_empty() -> None:
    ...
    assert text.startswith(
        ...
        "Weather Bot Phase 0A Hold State Closeout"
    )
    for heading in REQUIRED_SECTIONS:
        _section(text, heading)


def test_docs_static_held_state_closeout_scope_and_no_meg_modification() -> None:
    text = _read()
    required_phrases = [
        "docs/static-test-only/held-state-closeout-only",
        "This ticket does not modify `meg/`",
        "closes out the broader Weather Bot Phase 0A held-state context",
...
def test_source_fetching_track_closed_held_owner_decision_and_current_state() -> None:
    ...
    assert "Closed owner decision: hold_source_fetching_runtime_track" in _section(
        text, "Source-fetching track closeout state"
    )
    required_phrases = [
...
def test_stage2_runtime_metadata_supplied_only_fail_closed_and_paths() -> None:
    ...
    stage2_section = _section(text, "Stage 2 runtime metadata closeout state")
...
def test_blocked_work_and_reopen_conditions_appear() -> None:
    text = _read()
    blocked_section = _section(text, "Closed and blocked work")
...
def test_machine_checkable_parser_ignores_assignments_after_next_heading() -> None:
    ...
    assert _assignments(synthetic) == {"evidence status": {"held_state_closeout_recorded"}}


def test_recommended_next_track_meta_refresh_only_not_implementation() -> None:
    text = _read()
    recommended = _section(text, "Recommended next ticket")
    assert "Recommended next ticket: `weather_bot_phase0a_meta_refresh`" in recommended
    assert "docs/static-test-only after the Phase 0A hold-state closeout" in recommended
    assert "It must not implement source fetching" in recommended
    assert "must not create provider connectors" in recommended
    assert _assignments(text)["recommended next track"] == {"weather_bot_phase0a_meta_refresh"}
