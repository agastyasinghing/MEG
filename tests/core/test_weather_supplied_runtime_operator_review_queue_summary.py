...
MODULE_PATH = Path("meg/weather/stage2/supplied_runtime_operator_review_queue_summary.py")
TEST_PATH = Path("tests/core/test_weather_supplied_runtime_operator_review_queue_summary.py")
...
def _assert_blocked_with_reason(
    record: soqs.SuppliedRuntimeOperatorReviewQueueSummaryRecord,
    reason: str,
) -> None:
    result = soqs.validate_supplied_runtime_operator_review_queue_summary_record(record)
    assert result.passed is False
    assert result.severity is soqs.ValidationSeverity.BLOCKED
    assert reason in result.reasons


def test_enums_are_closed_sets() -> None:
    assert soqs.OperatorReviewQueueSummaryStatus.values() == frozenset(
        {
            "operator_review_queue_summary_recorded",
            "operator_review_queue_summary_missing",
            "operator_review_queue_summary_ambiguous",
            "operator_review_queue_summary_unsupported",
            "operator_review_queue_summary_unknown",
        }
    )
    assert soqs.OperatorReviewQueueSummaryCompletenessStatus.values() == frozenset(
        {
            "operator_review_queue_summary_complete",
            "operator_review_queue_summary_incomplete",
            "operator_review_queue_summary_ambiguous",
            "operator_review_queue_summary_unknown",
        }
    )
    assert soqs.OperatorReviewQueueSummaryPosture.values() == frozenset(
        {
            "operator_review_queue_summary_in_memory_only",
            "operator_review_queue_summary_missing",
            "operator_review_queue_summary_ambiguous",
            "operator_review_queue_summary_unsupported",
            "operator_review_queue_summary_unknown",
    ...
    assert soqs.OperatorReviewStatus.values() == frozenset(
        {
    ...
    assert soqs.RuntimeGateStatus.values() == frozenset(
        {
            "runtime_gate_ready",
    ...
    assert soqs.ValidationSeverity.values() == frozenset(
        {"passed", "caution", "failed", "blocked"}
    )


def test_dataclass_construction() -> None:
    record = _valid_operator_review_queue_summary(provenance_notes="operator supplied queue")
    ...
    assert record.operator_review_queue_summary_status is (
        soqs.OperatorReviewQueueSummaryStatus.OPERATOR_REVIEW_QUEUE_SUMMARY_RECORDED
    )
    assert record.provenance_notes == "operator supplied queue"


def test_mapping_construction_coerces_string_enums_and_nested_operator_review_queue_entry_mapping() -> None:
    record = soqs.supplied_runtime_operator_review_queue_summary_record_from_mapping(
        {
            "condition_id": "condition-1",
            "token_id": "token-1",
            "outcome": "Yes",
            "supplied_runtime_operator_review_queue_entry": asdict(
                _valid_operator_review_queue_entry()
            ),
            "queue_summary_id": "queue-packet-2",
            "queue_summary_text": "Caller supplied operator-review queue summary summary text.",
            "operator_review_summary": "Operator review is required before any later action.",
            "blocked_reason_summary": "",
            "operator_review_queue_summary_status": "operator_review_queue_summary_recorded",
            "operator_review_queue_summary_completeness_status": "operator_review_queue_summary_complete",
            "operator_review_queue_summary_posture": "operator_review_queue_summary_in_memory_only",
    ...
    assert record.operator_review_queue_summary_completeness_status is (
        soqs.OperatorReviewQueueSummaryCompletenessStatus.OPERATOR_REVIEW_QUEUE_SUMMARY_COMPLETE
    )
    assert record.operator_review_queue_summary_posture is (
        soqs.OperatorReviewQueueSummaryPosture.OPERATOR_REVIEW_QUEUE_SUMMARY_IN_MEMORY_ONLY
    )
    assert record.operator_review_status is soqs.OperatorReviewStatus.OPERATOR_REVIEW_REQUIRED
    assert record.runtime_gate_status is soqs.RuntimeGateStatus.RUNTIME_GATE_READY
    assert record.provenance_notes == "queue note"


def test_minimal_valid_operator_review_queue_summary_passes() -> None:
    result = soqs.validate_supplied_runtime_operator_review_queue_summary_record(
        _valid_operator_review_queue_summary()
    )

    assert result.passed is True
    assert result.severity is soqs.ValidationSeverity.PASSED
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("queue_summary_id", "queue_summary_id is missing"),
        ("queue_summary_text", "queue_summary_text is missing"),
...
def test_blank_blocked_reason_summary_is_allowed_for_passing_queue_summary() -> None:
    result = soqs.validate_supplied_runtime_operator_review_queue_summary_record(
        _valid_operator_review_queue_summary(blocked_reason_summary="")
    )
...
def test_blank_blocked_reason_summary_fails_when_queue_summary_is_otherwise_blocked() -> None:
    result = soqs.validate_supplied_runtime_operator_review_queue_summary_record(
        _valid_operator_review_queue_summary(queue_summary_text="  ", blocked_reason_summary="")
    )

    assert result.passed is False
    assert result.reasons == (
        "queue_summary_text is missing",
        "blocked_reason_summary is missing",
    )


def test_nested_invalid_supplied_runtime_operator_review_queue_entry_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_summary(
            supplied_runtime_operator_review_queue_entry=_valid_operator_review_queue_entry(
                queue_entry_summary="  "
        ...
        "supplied runtime operator-review queue entry validation failed",
    )


def test_condition_id_mismatch_with_supplied_runtime_operator_review_queue_entry_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_summary(
            supplied_runtime_operator_review_queue_entry=_valid_operator_review_queue_entry(
        ...
        "condition_id does not match supplied runtime operator-review queue entry",
    )


def test_token_id_mismatch_with_supplied_runtime_operator_review_queue_entry_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_summary(
            supplied_runtime_operator_review_queue_entry=_valid_operator_review_queue_entry(
                token_id="token-2"
        ...
        "token_id does not match supplied runtime operator-review queue entry",
    )


def test_outcome_mismatch_with_supplied_runtime_operator_review_queue_entry_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_summary(
            supplied_runtime_operator_review_queue_entry=_valid_operator_review_queue_entry(
                outcome="No"
        ...
        "outcome does not match supplied runtime operator-review queue entry",
    )


def test_operator_review_summary_mismatch_with_supplied_runtime_operator_review_queue_entry_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_summary(
...
@pytest.mark.parametrize(
    "operator_review_queue_summary_status",
    (
        soqs.OperatorReviewQueueSummaryStatus.OPERATOR_REVIEW_QUEUE_SUMMARY_MISSING,
        soqs.OperatorReviewQueueSummaryStatus.OPERATOR_REVIEW_QUEUE_SUMMARY_AMBIGUOUS,
        soqs.OperatorReviewQueueSummaryStatus.OPERATOR_REVIEW_QUEUE_SUMMARY_UNSUPPORTED,
        soqs.OperatorReviewQueueSummaryStatus.OPERATOR_REVIEW_QUEUE_SUMMARY_UNKNOWN,
    ),
)
def test_non_recorded_operator_review_queue_summary_statuses_fail_closed(
    operator_review_queue_summary_status: soqs.OperatorReviewQueueSummaryStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_summary(
            operator_review_queue_summary_status=operator_review_queue_summary_status,
            blocked_reason_summary="blocked",
        ),
        f"operator review queue summary status is {operator_review_queue_summary_status.value}",
    )
...
@pytest.mark.parametrize(
    "operator_review_queue_summary_completeness_status",
    (
        soqs.OperatorReviewQueueSummaryCompletenessStatus.OPERATOR_REVIEW_QUEUE_SUMMARY_INCOMPLETE,
        soqs.OperatorReviewQueueSummaryCompletenessStatus.OPERATOR_REVIEW_QUEUE_SUMMARY_AMBIGUOUS,
        soqs.OperatorReviewQueueSummaryCompletenessStatus.OPERATOR_REVIEW_QUEUE_SUMMARY_UNKNOWN,
    ),
)
def test_non_complete_operator_review_queue_summary_completeness_statuses_fail_closed(
    operator_review_queue_summary_completeness_status: soqs.OperatorReviewQueueSummaryCompletenessStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_summary(
            operator_review_queue_summary_completeness_status=operator_review_queue_summary_completeness_status,
            blocked_reason_summary="blocked",
        ),
        "operator review queue summary completeness status is "
        f"{operator_review_queue_summary_completeness_status.value}",
    )
...
@pytest.mark.parametrize(
    "operator_review_queue_summary_posture",
    (
        soqs.OperatorReviewQueueSummaryPosture.OPERATOR_REVIEW_QUEUE_SUMMARY_MISSING,
        soqs.OperatorReviewQueueSummaryPosture.OPERATOR_REVIEW_QUEUE_SUMMARY_AMBIGUOUS,
        soqs.OperatorReviewQueueSummaryPosture.OPERATOR_REVIEW_QUEUE_SUMMARY_UNSUPPORTED,
        soqs.OperatorReviewQueueSummaryPosture.OPERATOR_REVIEW_QUEUE_SUMMARY_UNKNOWN,
    ),
)
def test_non_in_memory_only_operator_review_queue_summary_postures_fail_closed(
    operator_review_queue_summary_posture: soqs.OperatorReviewQueueSummaryPosture,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_summary(
            operator_review_queue_summary_posture=operator_review_queue_summary_posture,
            blocked_reason_summary="blocked",
        ),
        f"operator review queue summary posture is {operator_review_queue_summary_posture.value}",
    )
...
@pytest.mark.parametrize(
    ...
    (
        soqs.OperatorReviewStatus.OPERATOR_REVIEW_MISSING,
        soqs.OperatorReviewStatus.OPERATOR_REVIEW_AMBIGUOUS,
        soqs.OperatorReviewStatus.OPERATOR_REVIEW_NOT_REQUIRED,
        soqs.OperatorReviewStatus.OPERATOR_REVIEW_UNKNOWN,
    ),
)
def test_non_required_operator_review_statuses_fail_closed(
    operator_review_status: soqs.OperatorReviewStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_summary(
...
@pytest.mark.parametrize(
    "runtime_gate_status",
    (
        soqs.RuntimeGateStatus.RUNTIME_GATE_BLOCKED,
        soqs.RuntimeGateStatus.RUNTIME_GATE_REQUIRES_MANUAL_REVIEW,
        soqs.RuntimeGateStatus.RUNTIME_GATE_UNKNOWN,
    ),
)
def test_non_ready_runtime_gate_statuses_fail_closed(
    runtime_gate_status: soqs.RuntimeGateStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_queue_summary(
