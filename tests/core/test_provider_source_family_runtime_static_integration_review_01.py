"""Static checks for Provider Source Family Runtime Static Integration Review."""
...
CANONICAL_ID = "PROVIDER-SOURCE-FAMILY-RUNTIME-STATIC-INTEGRATION-REVIEW-01"
...
PROVIDER_SOURCE_FAMILY_RECORD_FIELDS = {
    "condition_id",
    "token_id",
    "outcome",
    "source_identity",
...
ALLOWED_FUTURE_CONSUMPTION_POSTURES = {
    "read_provider_source_family_record_only",
    "require_validated_provider_source_family",
    "fail_closed_on_invalid_provider_source_family",
    "preserve_condition_id_token_id_outcome",
    "require_no_provider_execution",
    "require_compatible_source_family",
...
ALLOWED_ASSIGNMENTS = {
    ...
    "integration review status": {
        ...
        "review_only",
        "post_pr_266_provider_source_family_runtime_scaffold",
    },
    "current state posture": {
        "provider_source_family_runtime_scaffold_landed",
        "integration_not_implemented",
    },
    "provider source family artifact": {
        "provider_source_family_runtime_py",
        "ProviderSourceFamilyRecord",
        "ProviderSourceFamilyValidationResult",
        "validate_provider_source_family_record",
    },
    "provider source family record field": PROVIDER_SOURCE_FAMILY_RECORD_FIELDS,
    ...
    "conditional next track": {
        "provider_source_family_integration_review_revision_if_scope_too_broad",
...
def test_required_docs_static_review_posture_and_scope_boundaries() -> None:
    ...
    required_phrases = [
        ...
        "PR #266 added `meg/weather/stage2/provider_source_family_runtime.py`",
        "PR #266 added `ProviderSourceFamilyRecord`",
        "PR #266 added `ProviderSourceFamilyValidationResult`",
        "PR #266 added fail-closed validation through `validate_provider_source_family_record`",
        "does not modify `meg/`",
...
def test_machine_checkable_assignments_are_section_scoped_and_allowed() -> None:
    ...
    assert assignments["provider source family record field"] == PROVIDER_SOURCE_FAMILY_RECORD_FIELDS
    ...
    assert assignments["recommended next track"] == {"manual_review_gate_runtime_scaffold"}
