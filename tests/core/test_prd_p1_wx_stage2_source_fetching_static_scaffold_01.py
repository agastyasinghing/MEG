"""Static checks for Weather Bot narrow source-fetching static scaffold."""
...
CANONICAL_ID = "PRD-P1-WX-STAGE2-SOURCE-FETCHING-STATIC-SCAFFOLD-01"
ARTIFACT_PATH = REPO_ROOT / f"docs/prd/{CANONICAL_ID}.md"
MACHINE_HEADING = "## Machine-checkable source-fetching static-scaffold assignments"

REQUIRED_SECTIONS = (
    "Status and scope", "Relationship to narrow implementation plan",
    "Relationship to implementation approval decision", "Relationship to implementation approval request",
    "Relationship to Weather Bot PRD and architecture alignment", "Static scaffold objective",
    "Approved narrow scaffold scope", "Static scaffold surfaces", "Static source identity scaffold",
    "Static retrieval context scaffold", "Static provider/source family scaffold",
    "Static manual review gate scaffold", "Static no-lookahead metadata gate scaffold",
    "Static fail-closed validation scaffold", "Static audit surface scaffold", "Future static module boundaries",
    "Future static test boundaries", "Future static data-shape boundaries", "Fail-closed validation expectations",
    "No-lookahead and provenance expectations", "Runtime boundary", "Provider/source execution boundary",
    "Credential/config boundary", "Generated-data and fixture boundary", "Canonical identifier posture",
    ...
    "Recommended next ticket", "Machine-checkable source-fetching static-scaffold assignments", "Acceptance criteria",
)
RELATIONSHIP_IDS = {
    CANONICAL_ID,
...
ALLOWED_ASSIGNMENTS = {
    ...
    "static scaffold status": {"docs_static_test_only", "static_scaffold_only", "post_pr_256_narrow_implementation_plan"},
    "current state posture": {"static_scaffold_only", "runtime_not_approved"},
    "approved narrow scaffold scope": APPROVED_SCOPE,
    "static scaffold surface": SURFACES,
    "future static module": MODULES,
    "future static test": TESTS,
    "future static field": FIELDS,
    "fail closed expectation": FAILS,
    "not executed scope": NOT_EXECUTED,
    "provider source posture": {"provider_connectors_not_approved", "provider_clients_not_created", "live_provider_source_fetching_not_approved", "forecast_pulls_not_approved", "api_calls_not_approved", "scraping_not_approved", "provider_source_static_scaffold_only"},
...
def test_required_static_scaffold_posture_and_non_approval_language() -> None:
    text = _read()
    required = [
        "narrow source-fetching static scaffold artifact only", "docs/static-test-only/static-scaffold-only",
        "PR #256 is the latest completed narrow implementation-plan predecessor",
        "PR #256 recommended `narrow_source_fetching_static_scaffold`", "does not implement runtime source fetching",
        "does not modify runtime code", "does not modify Weather Bot runtime code", "does not create provider connectors",
    ...
    for value in (APPROVED_SCOPE | SURFACES | MODULES | TESTS | FIELDS | FAILS | NOT_EXECUTED | SOURCE_FAMILIES |
...
def test_machine_checkable_assignments_are_section_scoped_and_allowed() -> None:
    ...
    assert assignments["approved narrow scaffold scope"] == APPROVED_SCOPE
    assert assignments["static scaffold surface"] == SURFACES
    assert assignments["future static module"] == MODULES
    assert assignments["future static test"] == TESTS
    assert assignments["future static field"] == FIELDS
    assert assignments["fail closed expectation"] == FAILS
    assert assignments["not executed scope"] == NOT_EXECUTED
    assert assignments["recommended next track"] == {"narrow_source_fetching_static_scaffold_closeout"}
...
def test_synthetic_parser_scoping_ignores_assignments_after_next_heading() -> None:
    ...
    assert _assignments(synthetic) == {"recommended next track": {"narrow_source_fetching_static_scaffold_closeout"}}
