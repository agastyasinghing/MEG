...
REQUIRED_SECTIONS = (
    ...
    "Owner decision option representation", "Explicit non-approval representation",
...
def test_predecessor_exclusion_non_approvals_and_next_tracks_are_recorded() -> None:
    ...
    owner_options = {value for field, value in pairs if field == "owner decision option"}
    assert owner_options == set(CLOSED_SETS["owner decision option"])
    assert not any("implementation" in value or "execute" in value for value in owner_options)
