"""Static checks for Weather Bot Phase 0A market-contract inventory self-review."""
...
CANONICAL_ID = "WEATHER-BOT-PHASE0A-MARKET-CONTRACT-STATIC-INVENTORY-SELF-REVIEW-01"
ARTIFACT_PATH = REPO_ROOT / f"docs/prd/{CANONICAL_ID}.md"
TEST_PATH = REPO_ROOT / "tests/core/test_weather_bot_phase0a_market_contract_static_inventory_self_review_01.py"
MACHINE_HEADING = "Machine-checkable Weather Bot Phase 0A market-contract static-inventory self-review assignments"
NEXT_TRACK = "weather_bot_phase0a_market_contract_static_inventory_revision_if_scope_too_broad"
COMPLETE_TRACK = "weather_bot_phase0a_market_contract_static_inventory_self_review"
...
ALLOWED_ASSIGNMENTS = {
    ...
    "self review status": {
        ...
        "post_weather_bot_phase0a_market_contract_static_inventory",
    },
    ...
    "market contract field verified": MARKET_CONTRACT_FIELDS,
...
def test_document_exists_canonical_id_and_sections_are_non_empty() -> None:
    ...
    assert text.startswith(
        "# WEATHER-BOT-PHASE0A-MARKET-CONTRACT-STATIC-INVENTORY-SELF-REVIEW-01 — "
        "Weather Bot Phase 0A Market Contract Static Inventory Self-Review"
    )
...
def test_required_posture_and_boundaries_are_present() -> None:
    ...
    required_phrases = [
        ...
        "reviews `WEATHER-BOT-PHASE0A-MARKET-CONTRACT-STATIC-INVENTORY-01`",
...
def test_market_contract_fields_are_static_inventory_only() -> None:
    text = _read()
    section = _section(text, "Market contract field verification")
    assert "All market contract fields were verified as static inventory only" in section
    for field in MARKET_CONTRACT_FIELDS:
        assert f"`{field}`" in section


def test_canonical_routing_and_market_id_non_routing_are_preserved() -> None:
    section = _section(_read(), "Canonical routing verification")
    assert "Canonical routing remains limited to `condition_id`, `token_id`, and `outcome`" in section
