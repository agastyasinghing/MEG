...
MODULE_PATH = Path("meg/weather/stage2/supplied_runtime_operator_review_final_bundle.py")
TEST_PATH = Path("tests/core/test_weather_supplied_runtime_operator_review_final_bundle.py")
...
def _assert_blocked_with_reason(
    record: sofb.SuppliedRuntimeOperatorReviewFinalBundleRecord,
    reason: str,
) -> None:
    result = sofb.validate_supplied_runtime_operator_review_final_bundle_record(record)
    assert result.passed is False
    assert result.severity is sofb.ValidationSeverity.BLOCKED
    assert reason in result.reasons


def test_enums_are_closed_sets() -> None:
    assert sofb.OperatorReviewFinalBundleStatus.values() == frozenset(
        {
            "operator_review_final_bundle_recorded",
            "operator_review_final_bundle_missing",
            "operator_review_final_bundle_ambiguous",
            "operator_review_final_bundle_unsupported",
            "operator_review_final_bundle_unknown",
        }
    )
    assert sofb.OperatorReviewFinalBundleCompletenessStatus.values() == frozenset(
        {
            "operator_review_final_bundle_complete",
            "operator_review_final_bundle_incomplete",
            "operator_review_final_bundle_ambiguous",
            "operator_review_final_bundle_unknown",
        }
    )
    assert sofb.OperatorReviewFinalBundlePosture.values() == frozenset(
        {
            "operator_review_final_bundle_in_memory_only",
            "operator_review_final_bundle_missing",
            "operator_review_final_bundle_ambiguous",
            "operator_review_final_bundle_unsupported",
            "operator_review_final_bundle_unknown",
    ...
    assert sofb.OperatorReviewStatus.values() == frozenset(
        {
    ...
    assert sofb.RuntimeGateStatus.values() == frozenset(
        {
            "runtime_gate_ready",
    ...
    assert sofb.ValidationSeverity.values() == frozenset(
        {"passed", "caution", "failed", "blocked"}
    )


def test_dataclass_construction() -> None:
    record = _valid_operator_review_final_bundle(provenance_notes="operator supplied final bundle")
    ...
    assert record.operator_review_final_bundle_status is (
        sofb.OperatorReviewFinalBundleStatus.OPERATOR_REVIEW_FINAL_BUNDLE_RECORDED
    )
    assert record.provenance_notes == "operator supplied final bundle"


def test_mapping_construction_coerces_string_enums_and_nested_operator_review_final_packet_mapping() -> None:
    record = sofb.supplied_runtime_operator_review_final_bundle_record_from_mapping(
        {
            "condition_id": "condition-1",
            "token_id": "token-1",
            "outcome": "Yes",
            "supplied_runtime_operator_review_final_packet": asdict(
                _valid_operator_review_final_packet()
            ),
            "final_bundle_id": "final-bundle-2",
            "final_bundle_summary": "Caller supplied operator-review final bundle summary text.",
            "operator_review_summary": "Operator review is required before any later action.",
            "blocked_reason_summary": "",
            "operator_review_final_bundle_status": "operator_review_final_bundle_recorded",
            "operator_review_final_bundle_completeness_status": "operator_review_final_bundle_complete",
            "operator_review_final_bundle_posture": "operator_review_final_bundle_in_memory_only",
            "operator_review_status": "operator_review_required",
            "runtime_gate_status": "runtime_gate_ready",
            "provenance_notes": "final bundle note",
        }
    )
    ...
    assert record.operator_review_final_bundle_completeness_status is (
        sofb.OperatorReviewFinalBundleCompletenessStatus.OPERATOR_REVIEW_FINAL_BUNDLE_COMPLETE
    )
    assert record.operator_review_final_bundle_posture is (
        sofb.OperatorReviewFinalBundlePosture.OPERATOR_REVIEW_FINAL_BUNDLE_IN_MEMORY_ONLY
    )
    assert record.operator_review_status is sofb.OperatorReviewStatus.OPERATOR_REVIEW_REQUIRED
    assert record.runtime_gate_status is sofb.RuntimeGateStatus.RUNTIME_GATE_READY
    assert record.provenance_notes == "final bundle note"


def test_minimal_valid_operator_review_final_bundle_passes() -> None:
    result = sofb.validate_supplied_runtime_operator_review_final_bundle_record(
        _valid_operator_review_final_bundle()
    )

    assert result.passed is True
    assert result.severity is sofb.ValidationSeverity.PASSED
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("final_bundle_id", "final_bundle_id is missing"),
        ("final_bundle_summary", "final_bundle_summary is missing"),
...
def test_blank_blocked_reason_summary_is_allowed_for_passing_final_bundle() -> None:
    result = sofb.validate_supplied_runtime_operator_review_final_bundle_record(
        _valid_operator_review_final_bundle(blocked_reason_summary="")
    )
...
def test_blank_blocked_reason_summary_fails_when_final_bundle_is_otherwise_blocked() -> None:
    result = sofb.validate_supplied_runtime_operator_review_final_bundle_record(
        _valid_operator_review_final_bundle(final_bundle_summary="  ", blocked_reason_summary="")
    )

    assert result.passed is False
    assert result.reasons == (
        "final_bundle_summary is missing",
        "blocked_reason_summary is missing",
    )


def test_nested_invalid_supplied_runtime_operator_review_final_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_bundle(
            supplied_runtime_operator_review_final_packet=_valid_operator_review_final_packet(
        ...
        "supplied runtime operator-review final packet validation failed",
    )


def test_condition_id_mismatch_with_supplied_runtime_operator_review_final_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_bundle(
            supplied_runtime_operator_review_final_packet=_valid_operator_review_final_packet(
        ...
        "condition_id does not match supplied runtime operator-review final packet",
    )


def test_token_id_mismatch_with_supplied_runtime_operator_review_final_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_bundle(
            supplied_runtime_operator_review_final_packet=_valid_operator_review_final_packet(
                token_id="token-2"
        ...
        "token_id does not match supplied runtime operator-review final packet",
    )


def test_outcome_mismatch_with_supplied_runtime_operator_review_final_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_bundle(
            supplied_runtime_operator_review_final_packet=_valid_operator_review_final_packet(
                outcome="No"
        ...
        "outcome does not match supplied runtime operator-review final packet",
    )


def test_operator_review_summary_mismatch_with_supplied_runtime_operator_review_final_packet_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_bundle(
...
@pytest.mark.parametrize(
    "operator_review_final_bundle_status",
    (
        sofb.OperatorReviewFinalBundleStatus.OPERATOR_REVIEW_FINAL_BUNDLE_MISSING,
        sofb.OperatorReviewFinalBundleStatus.OPERATOR_REVIEW_FINAL_BUNDLE_AMBIGUOUS,
        sofb.OperatorReviewFinalBundleStatus.OPERATOR_REVIEW_FINAL_BUNDLE_UNSUPPORTED,
        sofb.OperatorReviewFinalBundleStatus.OPERATOR_REVIEW_FINAL_BUNDLE_UNKNOWN,
    ),
)
def test_non_recorded_operator_review_final_bundle_statuses_fail_closed(
    operator_review_final_bundle_status: sofb.OperatorReviewFinalBundleStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_bundle(
            operator_review_final_bundle_status=operator_review_final_bundle_status,
            blocked_reason_summary="blocked",
        ),
        f"operator review final bundle status is {operator_review_final_bundle_status.value}",
    )
...
@pytest.mark.parametrize(
    "operator_review_final_bundle_completeness_status",
    (
        sofb.OperatorReviewFinalBundleCompletenessStatus.OPERATOR_REVIEW_FINAL_BUNDLE_INCOMPLETE,
        sofb.OperatorReviewFinalBundleCompletenessStatus.OPERATOR_REVIEW_FINAL_BUNDLE_AMBIGUOUS,
        sofb.OperatorReviewFinalBundleCompletenessStatus.OPERATOR_REVIEW_FINAL_BUNDLE_UNKNOWN,
    ),
)
def test_non_complete_operator_review_final_bundle_completeness_statuses_fail_closed(
    operator_review_final_bundle_completeness_status: sofb.OperatorReviewFinalBundleCompletenessStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_bundle(
            operator_review_final_bundle_completeness_status=(
        ...
        f"{operator_review_final_bundle_completeness_status.value}",
    )
...
@pytest.mark.parametrize(
    "operator_review_final_bundle_posture",
    (
        sofb.OperatorReviewFinalBundlePosture.OPERATOR_REVIEW_FINAL_BUNDLE_MISSING,
        sofb.OperatorReviewFinalBundlePosture.OPERATOR_REVIEW_FINAL_BUNDLE_AMBIGUOUS,
        sofb.OperatorReviewFinalBundlePosture.OPERATOR_REVIEW_FINAL_BUNDLE_UNSUPPORTED,
        sofb.OperatorReviewFinalBundlePosture.OPERATOR_REVIEW_FINAL_BUNDLE_UNKNOWN,
    ),
)
def test_non_in_memory_only_operator_review_final_bundle_postures_fail_closed(
    operator_review_final_bundle_posture: sofb.OperatorReviewFinalBundlePosture,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_bundle(
            operator_review_final_bundle_posture=operator_review_final_bundle_posture,
            blocked_reason_summary="blocked",
        ),
        f"operator review final bundle posture is {operator_review_final_bundle_posture.value}",
    )
...
@pytest.mark.parametrize(
    ...
    (
        sofb.OperatorReviewStatus.OPERATOR_REVIEW_MISSING,
        sofb.OperatorReviewStatus.OPERATOR_REVIEW_AMBIGUOUS,
        sofb.OperatorReviewStatus.OPERATOR_REVIEW_NOT_REQUIRED,
        sofb.OperatorReviewStatus.OPERATOR_REVIEW_UNKNOWN,
    ),
)
def test_non_required_operator_review_statuses_fail_closed(
    operator_review_status: sofb.OperatorReviewStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_bundle(
...
@pytest.mark.parametrize(
    "runtime_gate_status",
    (
        sofb.RuntimeGateStatus.RUNTIME_GATE_BLOCKED,
        sofb.RuntimeGateStatus.RUNTIME_GATE_REQUIRES_MANUAL_REVIEW,
        sofb.RuntimeGateStatus.RUNTIME_GATE_UNKNOWN,
    ),
)
def test_non_ready_runtime_gate_statuses_fail_closed(
    runtime_gate_status: sofb.RuntimeGateStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_final_bundle(
