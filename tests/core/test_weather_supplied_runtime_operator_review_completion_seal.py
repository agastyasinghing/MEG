...
MODULE_PATH = Path("meg/weather/stage2/supplied_runtime_operator_review_completion_seal.py")
TEST_PATH = Path("tests/core/test_weather_supplied_runtime_operator_review_completion_seal.py")
...
def _assert_blocked_with_reason(
    record: socs.SuppliedRuntimeOperatorReviewCompletionSealRecord,
    reason: str,
) -> None:
    result = socs.validate_supplied_runtime_operator_review_completion_seal_record(record)
    assert result.passed is False
    assert result.severity is socs.ValidationSeverity.BLOCKED
    assert reason in result.reasons


def test_enums_are_closed_sets() -> None:
    assert socs.OperatorReviewCompletionSealStatus.values() == frozenset(
        {
            "operator_review_completion_seal_recorded",
            "operator_review_completion_seal_missing",
            "operator_review_completion_seal_ambiguous",
            "operator_review_completion_seal_unsupported",
            "operator_review_completion_seal_unknown",
        }
    )
    assert socs.OperatorReviewCompletionCompletenessStatus.values() == frozenset(
        {
            "operator_review_completion_complete",
            "operator_review_completion_incomplete",
            "operator_review_completion_ambiguous",
            "operator_review_completion_unknown",
        }
    )
    assert socs.OperatorReviewCompletionSealPosture.values() == frozenset(
        {
            "operator_review_completion_seal_in_memory_only",
            "operator_review_completion_seal_missing",
            "operator_review_completion_seal_ambiguous",
            "operator_review_completion_seal_unsupported",
            "operator_review_completion_seal_unknown",
    ...
    assert socs.OperatorReviewStatus.values() == frozenset(
        {
    ...
    assert socs.RuntimeGateStatus.values() == frozenset(
        {
            "runtime_gate_ready",
    ...
    assert socs.ValidationSeverity.values() == frozenset(
        {"passed", "caution", "failed", "blocked"}
    )


def test_dataclass_construction() -> None:
    record = _valid_operator_review_completion_seal(provenance_notes="operator supplied completion seal")
    ...
    assert record.operator_review_completion_seal_status is (
        socs.OperatorReviewCompletionSealStatus.OPERATOR_REVIEW_COMPLETION_SEAL_RECORDED
    )
    assert record.provenance_notes == "operator supplied completion seal"


def test_mapping_construction_coerces_string_enums_and_nested_operator_review_final_bundle_mapping() -> None:
    record = socs.supplied_runtime_operator_review_completion_seal_record_from_mapping(
        {
            "condition_id": "condition-1",
            "token_id": "token-1",
            "outcome": "Yes",
            "supplied_runtime_operator_review_final_bundle": asdict(
                _valid_operator_review_final_bundle()
            ),
            "completion_seal_id": "completion-seal-2",
            "completion_seal_summary": "Caller supplied operator-review completion seal summary text.",
            "operator_review_summary": "Operator review is required before any later action.",
            "blocked_reason_summary": "",
            "operator_review_completion_seal_status": "operator_review_completion_seal_recorded",
            "operator_review_completion_completeness_status": "operator_review_completion_complete",
            "operator_review_completion_seal_posture": "operator_review_completion_seal_in_memory_only",
            "operator_review_status": "operator_review_required",
            "runtime_gate_status": "runtime_gate_ready",
            "provenance_notes": "completion seal note",
        }
    )
    ...
    assert record.operator_review_completion_completeness_status is (
        socs.OperatorReviewCompletionCompletenessStatus.OPERATOR_REVIEW_COMPLETION_COMPLETE
    )
    assert record.operator_review_completion_seal_posture is (
        socs.OperatorReviewCompletionSealPosture.OPERATOR_REVIEW_COMPLETION_SEAL_IN_MEMORY_ONLY
    )
    assert record.operator_review_status is socs.OperatorReviewStatus.OPERATOR_REVIEW_REQUIRED
    assert record.runtime_gate_status is socs.RuntimeGateStatus.RUNTIME_GATE_READY
    assert record.provenance_notes == "completion seal note"


def test_minimal_valid_operator_review_completion_seal_passes() -> None:
    result = socs.validate_supplied_runtime_operator_review_completion_seal_record(
        _valid_operator_review_completion_seal()
    )

    assert result.passed is True
    assert result.severity is socs.ValidationSeverity.PASSED
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("completion_seal_id", "completion_seal_id is missing"),
        ("completion_seal_summary", "completion_seal_summary is missing"),
...
def test_blank_blocked_reason_summary_is_allowed_for_passing_completion_seal() -> None:
    result = socs.validate_supplied_runtime_operator_review_completion_seal_record(
        _valid_operator_review_completion_seal(blocked_reason_summary="")
    )
...
def test_blank_blocked_reason_summary_fails_when_completion_seal_is_otherwise_blocked() -> None:
    result = socs.validate_supplied_runtime_operator_review_completion_seal_record(
        _valid_operator_review_completion_seal(completion_seal_summary="  ", blocked_reason_summary="")
    )

    assert result.passed is False
    assert result.reasons == (
        "completion_seal_summary is missing",
        "blocked_reason_summary is missing",
    )


def test_nested_invalid_supplied_runtime_operator_review_final_bundle_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_completion_seal(
            supplied_runtime_operator_review_final_bundle=_valid_operator_review_final_bundle(
        ...
        "supplied runtime operator-review final bundle validation failed",
    )


def test_condition_id_mismatch_with_supplied_runtime_operator_review_final_bundle_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_completion_seal(
            supplied_runtime_operator_review_final_bundle=_valid_operator_review_final_bundle(
        ...
        "condition_id does not match supplied runtime operator-review final bundle",
    )


def test_token_id_mismatch_with_supplied_runtime_operator_review_final_bundle_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_completion_seal(
            supplied_runtime_operator_review_final_bundle=_valid_operator_review_final_bundle(
                token_id="token-2"
        ...
        "token_id does not match supplied runtime operator-review final bundle",
    )


def test_outcome_mismatch_with_supplied_runtime_operator_review_final_bundle_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_completion_seal(
            supplied_runtime_operator_review_final_bundle=_valid_operator_review_final_bundle(
                outcome="No"
        ...
        "outcome does not match supplied runtime operator-review final bundle",
    )


def test_operator_review_summary_mismatch_with_supplied_runtime_operator_review_final_bundle_fails_closed() -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_completion_seal(
...
@pytest.mark.parametrize(
    "operator_review_completion_seal_status",
    (
        socs.OperatorReviewCompletionSealStatus.OPERATOR_REVIEW_COMPLETION_SEAL_MISSING,
        socs.OperatorReviewCompletionSealStatus.OPERATOR_REVIEW_COMPLETION_SEAL_AMBIGUOUS,
        socs.OperatorReviewCompletionSealStatus.OPERATOR_REVIEW_COMPLETION_SEAL_UNSUPPORTED,
        socs.OperatorReviewCompletionSealStatus.OPERATOR_REVIEW_COMPLETION_SEAL_UNKNOWN,
    ),
)
def test_non_recorded_operator_review_completion_seal_statuses_fail_closed(
    operator_review_completion_seal_status: socs.OperatorReviewCompletionSealStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_completion_seal(
            operator_review_completion_seal_status=operator_review_completion_seal_status,
            blocked_reason_summary="blocked",
        ),
        f"operator review completion seal status is {operator_review_completion_seal_status.value}",
    )
...
@pytest.mark.parametrize(
    "operator_review_completion_completeness_status",
    (
        socs.OperatorReviewCompletionCompletenessStatus.OPERATOR_REVIEW_COMPLETION_INCOMPLETE,
        socs.OperatorReviewCompletionCompletenessStatus.OPERATOR_REVIEW_COMPLETION_AMBIGUOUS,
        socs.OperatorReviewCompletionCompletenessStatus.OPERATOR_REVIEW_COMPLETION_UNKNOWN,
    ),
)
def test_non_complete_operator_review_completion_completeness_statuses_fail_closed(
    operator_review_completion_completeness_status: socs.OperatorReviewCompletionCompletenessStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_completion_seal(
            operator_review_completion_completeness_status=(
        ...
        f"{operator_review_completion_completeness_status.value}",
    )
...
@pytest.mark.parametrize(
    "operator_review_completion_seal_posture",
    (
        socs.OperatorReviewCompletionSealPosture.OPERATOR_REVIEW_COMPLETION_SEAL_MISSING,
        socs.OperatorReviewCompletionSealPosture.OPERATOR_REVIEW_COMPLETION_SEAL_AMBIGUOUS,
        socs.OperatorReviewCompletionSealPosture.OPERATOR_REVIEW_COMPLETION_SEAL_UNSUPPORTED,
        socs.OperatorReviewCompletionSealPosture.OPERATOR_REVIEW_COMPLETION_SEAL_UNKNOWN,
    ),
)
def test_non_in_memory_only_operator_review_completion_seal_postures_fail_closed(
    operator_review_completion_seal_posture: socs.OperatorReviewCompletionSealPosture,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_completion_seal(
            operator_review_completion_seal_posture=operator_review_completion_seal_posture,
            blocked_reason_summary="blocked",
        ),
        f"operator review completion seal posture is {operator_review_completion_seal_posture.value}",
    )
...
@pytest.mark.parametrize(
    ...
    (
        socs.OperatorReviewStatus.OPERATOR_REVIEW_MISSING,
        socs.OperatorReviewStatus.OPERATOR_REVIEW_AMBIGUOUS,
        socs.OperatorReviewStatus.OPERATOR_REVIEW_NOT_REQUIRED,
        socs.OperatorReviewStatus.OPERATOR_REVIEW_UNKNOWN,
    ),
)
def test_non_required_operator_review_statuses_fail_closed(
    operator_review_status: socs.OperatorReviewStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_completion_seal(
...
@pytest.mark.parametrize(
    "runtime_gate_status",
    (
        socs.RuntimeGateStatus.RUNTIME_GATE_BLOCKED,
        socs.RuntimeGateStatus.RUNTIME_GATE_REQUIRES_MANUAL_REVIEW,
        socs.RuntimeGateStatus.RUNTIME_GATE_UNKNOWN,
    ),
)
def test_non_ready_runtime_gate_statuses_fail_closed(
    runtime_gate_status: socs.RuntimeGateStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_operator_review_completion_seal(
