"""Static checks for MEG-ARCH-ALIGN-07 compatibility-boundary review artifact.
...
ARTIFACT_PATH = REPO_ROOT / "docs/architecture/MEG-ARCH-ALIGN-07_COMPATIBILITY_BOUNDARY_REVIEW.md"
CANONICAL_ID = "MEG-ARCH-ALIGN-07"
MACHINE_HEADING = "## Machine-checkable compatibility-boundary review artifact assignments"
TABLE_HEADING = "Compatibility-boundary review table"
...
REQUIRED_PATHS = {
    "AGENTS.md",
    "docs/prd/MEG-ARCH-ALIGN-01_ARCHITECTURE_ALIGNMENT_PLANNING.md",
    "docs/prd/MEG-ARCH-ALIGN-02_MARKET_ID_INVENTORY_CLASSIFICATION_PLANNING.md",
    ...
    "tests/core/test_meg_arch_align_01.py",
    "tests/core/test_meg_arch_align_02.py",
    "tests/core/test_meg_arch_align_03.py",
...
PROPOSED_BOUNDARY_TYPES = {
    "keep_compatibility_boundary",
    "shrink_boundary_later",
    "plan_compatibility_shim_later",
    "reclassify_as_migration_candidate",
    "keep_frozen_doc",
    "keep_test_guard",
...
RECOMMENDED_BOUNDARY_ACTIONS = {
    "keep_as_documented_boundary",
    "shrink_after_approved_migration",
    "plan_compatibility_shim_later",
    "reclassify_for_migration_review",
    "leave_frozen_doc",
    "keep_test_guard",
...
ALLOWED_ASSIGNMENTS = {
    ...
    "review artifact status": {
        ...
        "compatibility_boundary_table_created",
    ...
    "review coverage status": {
        ...
        "frozen_docs_identified",
        "test_harness_guards_identified",
        "human_review_required_boundaries_identified",
    },
    ...
    "current boundary type": CURRENT_BOUNDARY_TYPES,
    "proposed boundary type": PROPOSED_BOUNDARY_TYPES,
    "compatibility pressure": COMPATIBILITY_PRESSURES,
    "recommended boundary action": RECOMMENDED_BOUNDARY_ACTIONS,
...
def _machine_section(text: str) -> str:
    ...
    assert match, "Machine-checkable compatibility-boundary review assignment section is missing"
    section = match.group("section")
    assert section.strip(), "Machine-checkable compatibility-boundary review assignment section is empty"
    return section
...
def _table_rows(text: str) -> list[dict[str, str]]:
    ...
    assert len(table_lines) >= 3, "Compatibility-boundary review table is missing rows"
...
def test_review_artifact_docs_static_only_scope_is_stated() -> None:
    ...
    assert "documentation/static-evidence" in text
...
def test_compatibility_boundary_review_table_has_exact_columns_and_required_rows() -> None:
...
def test_table_closed_set_values_are_allowed() -> None:
    for row in _table_rows(_read_artifact()):
        assert row["current_boundary_type"] in CURRENT_BOUNDARY_TYPES
        assert row["proposed_boundary_type"] in PROPOSED_BOUNDARY_TYPES
        assert row["compatibility_pressure"] in COMPATIBILITY_PRESSURES
        assert row["recommended_boundary_action"] in RECOMMENDED_BOUNDARY_ACTIONS
        assert row["risk_level"] in RISK_LEVELS


def test_boundary_summary_sections_appear() -> None:
    text = _read_artifact()
    keep_section = _section(text, "Boundaries to keep temporarily")
    shrink_section = _section(text, "Boundaries to shrink later")
    human_review_section = _section(text, "Boundaries requiring human review")
    assert "kept temporarily" in keep_section or "kept" in keep_section
    assert "later shrinkage" in shrink_section or "Shrinking later" in shrink_section
    assert "Human review is required" in human_review_section
    assert "database migration boundaries" in human_review_section
...
def test_machine_checkable_assignment_section_exists_and_is_section_scoped() -> None:
    ...
    assert "- architecture alignment stage: compatibility_boundary_review_artifact" in section
