"""Static checks for Weather Bot narrow source-fetching runtime static scaffold."""
...
CANONICAL_ID = "PRD-P1-WX-STAGE2-SOURCE-FETCHING-RUNTIME-STATIC-SCAFFOLD-01"
ARTIFACT_PATH = REPO_ROOT / f"docs/prd/{CANONICAL_ID}.md"
MACHINE_HEADING = "## Machine-checkable source-fetching runtime static-scaffold assignments"

REQUIRED_SECTIONS = (
    "Status and scope", "Relationship to runtime implementation plan", "Relationship to runtime approval decision",
    "Relationship to runtime approval request", "Relationship to static scaffold closeout",
    "Relationship to Weather Bot PRD and architecture alignment", "Runtime static scaffold objective",
    "Approved scaffold authority", "Runtime static scaffold surfaces", "Source identity runtime static scaffold",
    "Retrieval context runtime static scaffold", "Provider/source family runtime static scaffold",
    "Manual review gate runtime static scaffold", "No-lookahead metadata runtime static scaffold",
    "Fail-closed validation runtime static scaffold", "Static audit runtime scaffold", "Future module names",
    "Future data-shape names", "Future test names", "Future fail-closed expectations",
    "Scope not implemented by this scaffold", "Scope not approved for execution", "Provider/source execution boundary",
    "Credential/config boundary", "Generated-data and fixture boundary", "Trading/autonomy/production boundary",
    "Canonical identifier posture", "Provider/source compatibility posture", "Offline-ingestion boundary posture",
    "Test-scope posture", "Risk and failure-mode posture", "Explicit non-execution boundaries",
    "Blocked implementation work", "Recommended next ticket",
    "Machine-checkable source-fetching runtime static-scaffold assignments", "Acceptance criteria",
)
RELATIONSHIP_IDS = {
    CANONICAL_ID,
...
SCAFFOLD_SURFACES = {
    "source_identity_recording_runtime_scaffold", "retrieval_context_recording_runtime_scaffold",
    "provider_source_family_recording_runtime_scaffold", "manual_review_gate_runtime_scaffold",
    "no_lookahead_metadata_gate_runtime_scaffold", "fail_closed_validation_gate_runtime_scaffold",
    "static_audit_surface_runtime_scaffold",
}
MODULE_NAMES = {
    "source_identity_runtime", "retrieval_context_runtime", "provider_source_family_runtime",
    "manual_review_gate_runtime", "no_lookahead_metadata_runtime", "fail_closed_validation_runtime",
...
FUTURE_TESTS = {
    "test_source_identity_runtime", "test_retrieval_context_runtime", "test_provider_source_family_runtime",
    "test_manual_review_gate_runtime", "test_no_lookahead_metadata_runtime", "test_fail_closed_validation_runtime",
...
ALLOWED_ASSIGNMENTS = {
    ...
    "conditional next track": {"runtime_static_scaffold_closeout_if_needed", "hold_checkpoint_if_runtime_code_not_desired", "runtime_static_scaffold_revision_if_scope_too_broad"},
...
def test_required_static_scaffold_posture_and_non_approval_language() -> None:
    text = _read()
    required = [
        "runtime static scaffold only", "docs/static-test-only/static-scaffold-only",
        "post-PR #261 runtime implementation-plan state",
        "PR #261 created `PRD-P1-WX-STAGE2-SOURCE-FETCHING-RUNTIME-IMPLEMENTATION-PLAN-01`",
        "PR #261 recommended `narrow_source_fetching_runtime_static_scaffold`",
        ...
        "No routing on `market_id` is introduced or approved", "Recommended next track: `first_real_code_source_identity_runtime_scaffold`",
    ]
    for phrase in required:
        assert phrase in text
    for value in SCAFFOLD_SURFACES | MODULE_NAMES | FIELD_NAMES | FUTURE_TESTS | FAIL_CLOSED | NOT_EXECUTED_SCOPE | SOURCE_FAMILIES | RETRIEVAL_MODES | ACCESS_METHODS:
...
def test_machine_checkable_assignments_are_section_scoped_and_allowed() -> None:
    ...
    assert assignments["approved scaffold authority"] == {"approve_narrow_source_fetching_runtime_implementation_plan"}
    assert assignments["runtime static scaffold surface"] == SCAFFOLD_SURFACES
    assert assignments["future module name"] == MODULE_NAMES
    assert assignments["future field name"] == FIELD_NAMES
    assert assignments["future test name"] == FUTURE_TESTS
    assert assignments["future fail closed expectation"] == FAIL_CLOSED
    assert assignments["not executed scope"] == NOT_EXECUTED_SCOPE
    assert assignments["recommended next track"] == {"first_real_code_source_identity_runtime_scaffold"}
...
def test_synthetic_parser_scoping_ignores_assignments_after_next_heading() -> None:
    ...
    assert _assignments(synthetic) == {"recommended next track": {"first_real_code_source_identity_runtime_scaffold"}}
