...
EXPECTED_MAPPING_ORDER_PHRASES = ['collections.abc.Mapping', 'MISSING_REQUIRED_FIELD` for every absent required key', 'UNEXPECTED_FIELD` for every unexpected key', 'exact built-in string keys in lexical order', 'non-string keys afterward in original mapping iteration order', 'required and nullable text, role, applicability modes, assignment status, overlap posture, integer, fixed postures, timestamps, general temporal checks, assigned role-specific temporal checks, status consistency, provenance, and supersession order', 'preserve every repeated code occurrence; perform no final sorting, filtering, insertion, set conversion, or deduplication', 'if any code exists, return `(None, blocked_result)`', 'construct `StrictOOSSplitAssignment` only when the complete code sequence is empty']
EXPECTED_DIRECT_APPLICABILITY_PHRASES = ['type(record.applicability_modes) is tuple', 'nonempty tuple', 'exact `SplitApplicabilityMode` member', 'enum subclasses, strings, unrelated enums', 'SplitApplicabilityMode.PRIMARY_TEMPORAL` must be exactly first', 'no entry may occur more than once', 'caller order is preserved', 'exactly one `INVALID_APPLICABILITY_MODES`', 'no tuple/list conversion', 'no enum-string adaptation']
EXPECTED_DIRECT_PROVENANCE_PHRASES = ['Direct provenance requires `type(record.provenance_refs) is tuple`', 'non-tuple container appends exactly one `INVALID_PROVENANCE_REF`', 'empty appends exactly one `EMPTY_PROVENANCE_REFS`', 'every entry must be an exact built-in nonblank string', 'one `INVALID_PROVENANCE_REF` in tuple order', 'duplicate valid references are allowed', 'no sorting, deduplication, conversion, normalization, or generated provenance occurs', 'container failure prevents entry iteration', 'emptiness and entry errors are mutually exclusive']
EXPECTED_SUPERSESSION_PHRASES = ['supersedes_split_assignment_id is None` is accepted', 'must already be an exact nonblank built-in string', 'BLANK_REQUIRED_TEXT` after required-text checks', 'SELF_SUPERSESSION` exactly once', 'supersedes_split_assignment_id == split_assignment_id', 'Do not append `SELF_SUPERSESSION` when either identifier is text-invalid', 'distinct valid superseded identifier is accepted', 'No identifier is generated or rewritten']
EXPECTED_DIRECT_ORDER_PHRASES = ['Complete direct-record validation order', 'all `BLANK_REQUIRED_TEXT` occurrences', 'INVALID_SPLIT_ROLE', 'INVALID_APPLICABILITY_MODES', 'INVALID_ASSIGNMENT_STATUS', 'INVALID_OVERLAP_CONTROL_POSTURE', 'INVALID_INTEGER_FIELD', 'primary split, tuning, calibration, baseline parity order', 'all `INVALID_TIMESTAMP` occurrences', 'general temporal codes', 'assigned role-specific temporal/label codes', 'assignment-status consistency codes', 'EMPTY_PROVENANCE_REFS', 'all `INVALID_PROVENANCE_REF` occurrences', 'SELF_SUPERSESSION']
EXPECTED_COLLECTION_ORDER_PHRASES = ['Root behavior: non-tuple root returns only `INVALID_ASSIGNMENT_COLLECTION_TYPE`', 'empty tuple returns only `EMPTY_ASSIGNMENT_COLLECTION`', 'validates each element in tuple order', 'complete direct-record validation codes in tuple order', 'DUPLICATE_ASSIGNMENT_ID', 'DUPLICATE_FOLD_RECORD_ASSIGNMENT', 'DUPLICATE_TEST_RECORD', 'INCONSISTENT_SPLIT_ID', 'INCONSISTENT_SPLIT_VERSION', 'INCONSISTENT_FOLD_DEFINITION', 'NON_MONOTONIC_FOLD_CUTOFF', 'LEAKAGE_GROUP_ROLE_CONFLICT']
EXPECTED_MAPPING_TEST_CASES = ['multiple missing required keys', 'multiple unexpected built-in string keys', 'multiple non-string unexpected keys', 'missing plus unexpected keys', 'missing plus malformed present text', 'unexpected plus invalid enums', 'malformed applicability modes plus invalid integer', 'timestamp and provenance failures', 'repeated identical validation codes', 'shape and temporal failures together', 'complete tuple equality']
...
REQUIRED_ORACLE_NAMES = [
    ...
    "EXPECTED_MAPPING_ORDER_PHRASES", "EXPECTED_DIRECT_APPLICABILITY_PHRASES", "EXPECTED_DIRECT_PROVENANCE_PHRASES",
    "EXPECTED_SUPERSESSION_PHRASES", "EXPECTED_DIRECT_ORDER_PHRASES", "EXPECTED_COLLECTION_ORDER_PHRASES",
    "EXPECTED_MAPPING_TEST_CASES",
]
...
def validate_direct_applicability(sections):
    section = sections["Exact single-record validation matrix"]
    for phrase in EXPECTED_DIRECT_APPLICABILITY_PHRASES:
        assert phrase in section

def validate_direct_provenance(sections):
    section = sections["Exact single-record validation matrix"]
    for phrase in EXPECTED_DIRECT_PROVENANCE_PHRASES:
        assert phrase in section

def validate_supersession(sections):
    section = sections["Exact single-record validation matrix"]
    for phrase in EXPECTED_SUPERSESSION_PHRASES:
        assert phrase in section

def validate_direct_record_order(sections):
    section = sections["Exact validation-order contract"]
    for phrase in EXPECTED_DIRECT_ORDER_PHRASES:
        assert phrase in section

def validate_mapping_order(sections):
    section = sections["Exact validation-order contract"]
    for phrase in EXPECTED_MAPPING_ORDER_PHRASES:
        assert phrase in section

def validate_collection_order(sections):
    section = sections["Exact collection-validation matrix"]
    for phrase in EXPECTED_COLLECTION_ORDER_PHRASES:
        assert phrase in section

def validate_mapping_order_test_requirements(sections):
    section = sections["Exact future test matrix"]
    for phrase in EXPECTED_MAPPING_TEST_CASES:
        assert phrase in section
...
def validate_heading_categories():
    heading_validators = {
        "Status and request scope": [validate_exact_section_bodies],
        "Actual PR #368 merge predecessor": [validate_exact_section_bodies],
        "Controlling strict-OOS planning contract": [validate_exact_section_bodies],
        "Requested future implementation slice identity": [validate_exact_section_bodies],
        "Exact future changed-file matrix": [validate_exact_section_bodies, validate_future_changed_file_table],
        "Exact future public-symbol matrix": [validate_exact_section_bodies, validate_public_symbol_table],
        "Exact future enum matrices": [validate_exact_section_bodies, validate_enum_section],
        "Exact future record-field matrix": [validate_exact_section_bodies, validate_record_result_section],
        "Exact fixed-posture matrix": [validate_exact_section_bodies, validate_fixed_postures],
        "Exact mapping-input matrix": [validate_exact_section_bodies, validate_mapping_section],
        "Exact single-record validation matrix": [validate_exact_section_bodies, validate_direct_applicability, validate_direct_provenance, validate_supersession],
        "Exact collection-validation matrix": [validate_exact_section_bodies, validate_collection_order],
        "Exact validation-code matrix": [validate_exact_section_bodies, validate_validation_codes],
        "Exact validation-order contract": [validate_exact_section_bodies, validate_direct_record_order, validate_mapping_order],
        "Exact future test matrix": [validate_exact_section_bodies, validate_mapping_order_test_requirements],
        "Dependency and import boundary": [validate_exact_section_bodies],
        "Canonical routing boundary": [validate_exact_section_bodies],
        "Temporal and no-lookahead boundary": [validate_exact_section_bodies],
        "Leakage-group and fold boundary": [validate_exact_section_bodies],
        "Assignment and exclusion semantics": [validate_exact_section_bodies],
        "Baseline parity boundary": [validate_exact_section_bodies],
        "Explicit future implementation non-goals": [validate_exact_section_bodies],
        "Approval decision options": [validate_exact_section_bodies, validate_decisions],
        "Current request status": [validate_exact_section_bodies],
        "Human decision and separate-approval boundary": [validate_exact_section_bodies],
        "Fail-closed requirements": [validate_exact_section_bodies],
        "Explicit non-approvals": [validate_exact_section_bodies],
        "Machine-checkable assignments": [validate_exact_section_bodies, validate_machine],
        "Acceptance criteria": [validate_exact_section_bodies],
    }
    assert list(heading_validators.keys()) == EXPECTED_HEADINGS
    assigned = []
    for category, headings in EXPECTED_HEADING_CATEGORIES.items():
        assert headings
        for heading in headings:
            assert heading in heading_validators
            assigned.append(heading)
    assert len(assigned) == len(set(assigned))
    assert set(assigned) == set(EXPECTED_HEADINGS)
    for heading in EXPECTED_HEADINGS:
        assert validate_exact_section_bodies in heading_validators[heading]
        assert heading_validators[heading]
...
def validate_allowlist():
    ...
    for path in EXPECTED_ALLOWLIST_PATHS:
        registry_counts[path] = 0
        mapping_counts[path] = 0
        ...
        if "_ARCH_ALIGN_03_DOWNSTREAM_ARTIFACTS" in target_names:
            for element in value_node.elts:
                if isinstance(element, ast.Constant) and element.value in EXPECTED_ALLOWLIST_PATHS:
                    registry_counts[element.value] += 1
        if "ALLOWED_MARKET_ID_OCCURRENCE_LINES" in target_names:
            ...
            for key, value in zip(mapping.keys, mapping.values):
                if isinstance(key, ast.Constant) and key.value in EXPECTED_ALLOWLIST_PATHS:
                    mapping_counts[key.value] += 1
                    assert isinstance(value, ast.Constant)
                    assert isinstance(value.value, int)
                    mapped_values[key.value] = value.value
    for path in EXPECTED_ALLOWLIST_PATHS:
        assert registry_counts[path] == 1
        assert mapping_counts[path] == 1
        observed = 0
        for line in Path(path).read_text().splitlines():
            if "market_id" in line:
                observed += 1
        assert mapped_values[path] == observed
...
def validate_complete(text):
    ...
    validate_direct_applicability(sections)
    validate_direct_provenance(sections)
    validate_supersession(sections)
    validate_direct_record_order(sections)
    validate_mapping_order(sections)
    validate_collection_order(sections)
    validate_mapping_order_test_requirements(sections)
...
def _assert_rejected(mutated, intended_validator):
    ...
    try:
        ...
        raise AssertionError("exact section validator accepted mutation")
    try:
...
def test_structural_mutations_are_rejected():
    ...
    cases = [
        ...
        ("Exact single-record validation matrix", validate_direct_applicability, "Direct applicability modes must be a nonempty tuple", "Direct applicability modes may be empty"),
        ("Exact single-record validation matrix", validate_direct_applicability, "SplitApplicabilityMode.PRIMARY_TEMPORAL` must be exactly first", "SplitApplicabilityMode.PRIMARY_TEMPORAL` may be anywhere"),
        ("Exact single-record validation matrix", validate_direct_applicability, "no entry may occur more than once", "entries may occur more than once"),
        ("Exact single-record validation matrix", validate_direct_provenance, "Direct provenance requires `type(record.provenance_refs) is tuple`", "Direct provenance permits iterables"),
        ("Exact single-record validation matrix", validate_direct_provenance, "empty appends exactly one `EMPTY_PROVENANCE_REFS`", "empty is accepted"),
        ("Exact single-record validation matrix", validate_direct_provenance, "every entry must be an exact built-in nonblank string", "entries may be coerced"),
        ("Exact single-record validation matrix", validate_supersession, "supersedes_split_assignment_id == split_assignment_id", "supersedes_split_assignment_id != split_assignment_id"),
        ("Exact validation-order contract", validate_mapping_order, "MISSING_REQUIRED_FIELD` for every absent required key in `EXPECTED_MAPPING_REQUIRED_KEYS` order", "MISSING_REQUIRED_FIELD` after unexpected keys"),
        ("Exact validation-order contract", validate_mapping_order, "exact built-in string keys in lexical order and non-string keys afterward", "non-string keys first"),
        ("Exact validation-order contract", validate_mapping_order, "required and nullable text, role, applicability modes", "role, required and nullable text, applicability modes"),
        ("Exact validation-order contract", validate_mapping_order, "preserve every repeated code occurrence; perform no final sorting, filtering, insertion, set conversion, or deduplication", "deduplicate final codes"),
        ("Exact validation-order contract", validate_mapping_order, "if any code exists, return `(None, blocked_result)`", "return a partial record"),
    ]
...
def test_static_source_safety():
    ...
    blocked_attrs = ["open", "write_text", "write_bytes", "touch", "unlink", "rename", "replace", "mkdir", "rmdir", "chmod", "symlink_to", "hardlink_to"]
    for node in ast.walk(tree):
