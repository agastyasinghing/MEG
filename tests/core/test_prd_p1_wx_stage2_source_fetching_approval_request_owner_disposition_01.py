"""Static checks for Weather Bot source-fetching owner disposition."""
...
ARTIFACT_PATH = REPO_ROOT / "docs/prd/PRD-P1-WX-STAGE2-SOURCE-FETCHING-APPROVAL-REQUEST-OWNER-DISPOSITION-01.md"
CANONICAL_ID = "PRD-P1-WX-STAGE2-SOURCE-FETCHING-APPROVAL-REQUEST-OWNER-DISPOSITION-01"
MACHINE_HEADING = "## Machine-checkable source-fetching approval-request owner-disposition assignments"

REQUIRED_SECTIONS = (
    "Status and scope", "Relationship to owner-disposition planning", "Relationship to meta refresh",
    "Relationship to hold checkpoint", "Relationship to source-fetching approval-request draft",
    "Relationship to source-fetching approval-request planning sequence",
    "Relationship to provider/source compatibility sequence",
    "Relationship to Weather Bot PRD and architecture alignment", "Owner disposition objective",
    "Owner disposition decision", "Decision rationale", "Decision scope", "What this disposition allows",
    "What this disposition does not allow", "Next allowed planning track", "Approval posture",
    "Planning-only posture", "Canonical identifier posture", "Source identity and provenance posture",
    ...
    "Recommended next ticket", "Machine-checkable source-fetching approval-request owner-disposition assignments",
    "Acceptance criteria",
)
RELATIONSHIP_IDS = {
    CANONICAL_ID,
...
ALLOWED_ASSIGNMENTS = {
    ...
    "owner disposition status": {"docs_static_test_only", "owner_disposition_recorded", "post_pr_249_owner_disposition_planning"},
    ...
    "approval request posture": {"narrow_source_fetching_planning_allowed", "source_fetching_implementation_not_approved", "provider_connector_implementation_not_approved", "later_explicit_implementation_approval_required"},
...
def test_required_posture_decision_and_non_approval_language() -> None:
    text = _read()
    required = [
        "owner-disposition only", "docs/static-test-only", "PR #249 is merged",
        "current state entering this ticket is `hold_checkpoint`",
        "no separate human-review ticket is pending by default",
        "actual selected owner-disposition decision is exactly `approve_narrow_source_fetching_planning_only`",
        "allows only a future docs/static-test-only narrow source-fetching implementation-planning proposal",
    ...
    for value in CLOSED_SET_VALUES | NON_APPROVED_BEHAVIORS | OWNER_DECISIONS | DISALLOWED_DECISIONS:
...
def test_machine_assignments_are_section_scoped_and_allowed() -> None:
    ...
    assert assignments["owner disposition decision"] == {"approve_narrow_source_fetching_planning_only"}
    assert assignments["recommended next track"] == {"narrow_source_fetching_planning_request"}
...
def test_synthetic_parser_scoping_ignores_assignments_after_next_heading() -> None:
    ...
    assert _assignments(synthetic) == {"recommended next track": {"narrow_source_fetching_planning_request"}}
