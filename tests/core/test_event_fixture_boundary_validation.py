"""Tests for the Phase 0A test-only shared-event fixture boundary."""
...
from tests.core.event_fixture_boundary import validate_test_event_boundary_payload

LEGACY_ID_FIELD = "market" + "_id"
LEGACY_ID_VALUE = "polymarket-presidential-election-2028"
CONDITION_ID_VALUE = "0xconditionphase0a02c"
...
def test_boundary_validates_realistic_raw_whale_trade_payload() -> None:
    event = validate_test_event_boundary_payload(_raw_whale_trade_payload())

    assert isinstance(event, RawWhaleTrade)
    assert event.event_type == "raw_whale_trade"
    assert event.wallet_address.startswith("0x")
    assert event.condition_id == CONDITION_ID_VALUE
    assert event.token_id == TOKEN_ID_VALUE


def test_boundary_validates_realistic_signal_payload() -> None:
    event = validate_test_event_boundary_payload(_signal_payload())

    assert isinstance(event, SignalEvent)
    assert event.event_type == "signal"
    assert event.signal_id == "sig-phase0a-02c-001"
    assert event.scores.lead_lag == pytest.approx(0.82)


def test_missing_schema_version_defaults_to_one_through_boundary() -> None:
    payload = _raw_whale_trade_payload()
    payload.pop("schema_version", None)

    event = validate_test_event_boundary_payload(payload)
...
def test_explicit_schema_version_one_round_trips_through_boundary() -> None:
...
def test_unsupported_schema_version_is_rejected_at_boundary() -> None:
    payload = _raw_whale_trade_payload()
    ...
    with pytest.raises(ValueError, match="Unsupported event schema_version"):
        validate_test_event_boundary_payload(payload)


def test_missing_event_type_is_rejected_at_boundary() -> None:
    payload = _raw_whale_trade_payload()
...
def test_unknown_event_type_is_rejected_at_boundary() -> None:
    payload = _raw_whale_trade_payload()
...
def test_boundary_dispatches_by_event_type_not_legacy_identifier() -> None:
    payload = _raw_whale_trade_payload()
...
def test_boundary_preserves_legacy_compatibility_fields() -> None:
    event = validate_test_event_boundary_payload(_signal_payload())
...
def test_boundary_does_not_require_canonical_identifiers_yet() -> None:
    payload = _signal_payload()
