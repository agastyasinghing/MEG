...
MODULE_PATH = Path("meg/weather/stage2/fail_closed_validation_runtime.py")
TEST_PATH = Path("tests/core/test_weather_fail_closed_validation_runtime.py")
...
def test_enums_are_closed_sets() -> None:
    assert fcvr.AggregateValidationStatus.values() == frozenset(
        {
            "aggregate_validation_passed",
            "aggregate_validation_failed",
            "aggregate_validation_blocked",
            "aggregate_validation_unknown",
        }
    )
    assert fcvr.DependencyValidationStatus.values() == frozenset(
        {
    ...
    assert fcvr.FailClosedPosture.values() == frozenset(
        {
    ...
    assert fcvr.RuntimeGateStatus.values() == frozenset(
        {
            "runtime_gate_ready",
    ...
    assert fcvr.ValidationSeverity.values() == frozenset(
        {"passed", "caution", "failed", "blocked"}
    )


def test_dataclass_construction_and_valid_record_passes() -> None:
    record = _valid_record(provenance_notes="supplied metadata only")
    assert record.provenance_notes == "supplied metadata only"
    result = fcvr.validate_fail_closed_validation_record(record)
    assert result == fcvr.FailClosedValidationResult(
        severity=fcvr.ValidationSeverity.PASSED, passed=True
    )


def test_mapping_construction_with_nested_mappings() -> None:
    record = fcvr.fail_closed_validation_record_from_mapping(_valid_mapping())
    ...
    assert (
        ...
        is fcvr.AggregateValidationStatus.AGGREGATE_VALIDATION_PASSED
    )
...
def test_mapping_construction_accepts_built_dependency_records() -> None:
    ...
    record = fcvr.fail_closed_validation_record_from_mapping(
        _valid_mapping(
            source_identity=source_identity,
            retrieval_context=retrieval_context,
            provider_source_family=provider_source_family,
...
@pytest.mark.parametrize(
    ("dependency_name", "record", "reason"),
    [
        (
            "source_identity",
            _valid_source_identity(condition_id="other"),
            "condition_id does not match source identity",
        ),
        (
            "retrieval_context",
            _valid_retrieval_context(condition_id="other"),
            "condition_id does not match retrieval context",
        ),
        (
            "provider_source_family",
            _valid_provider_source_family(**{"condition_id": "other"}),
            "condition_id does not match provider source family",
        ),
        (
            "manual_review_gate",
            _valid_manual_review_gate(**{"condition_id": "other"}),
            "condition_id does not match manual review gate",
        ),
        (
            "no_lookahead_metadata",
            _valid_no_lookahead_metadata(condition_id="other"),
            "condition_id does not match no-lookahead metadata",
        ),
    ],
)
def test_canonical_condition_mismatches_fail_closed(
    dependency_name: str, record: object, reason: str
) -> None:
    _assert_blocked_with_reason(_valid_record(**{dependency_name: record}), reason)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "source_identity",
            _valid_source_identity(source_id=" "),
            "source identity validation failed",
        ),
        (
            "retrieval_context",
            _valid_retrieval_context(accessed_at_utc=" "),
            "retrieval context validation failed",
        ),
        (
            "provider_source_family",
            _valid_provider_source_family(source_family=" "),
            "provider source family validation failed",
        ),
        (
            "manual_review_gate",
            _valid_manual_review_gate(reviewer_id=" "),
            "manual review gate validation failed",
        ),
        (
            "no_lookahead_metadata",
            _valid_no_lookahead_metadata(available_at_utc=" "),
            "no-lookahead metadata validation failed",
        ),
    ],
)
def test_invalid_nested_dependency_records_fail_closed(
    dependency_name: str, record: object, reason: str
) -> None:
    _assert_blocked_with_reason(_valid_record(**{dependency_name: record}), reason)
...
@pytest.mark.parametrize(
    ...
    [
        s
        for s in fcvr.AggregateValidationStatus
        if s is not fcvr.AggregateValidationStatus.AGGREGATE_VALIDATION_PASSED
    ],
)
def test_non_passing_aggregate_statuses_fail_closed(
    status: fcvr.AggregateValidationStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_record(aggregate_validation_status=status),
        f"aggregate validation status is {status.value}",
    )
...
@pytest.mark.parametrize(
    ...
    [
        s
        for s in fcvr.DependencyValidationStatus
        if s is not fcvr.DependencyValidationStatus.ALL_DEPENDENCIES_VALIDATED
    ],
)
def test_non_validated_dependency_statuses_fail_closed(
    status: fcvr.DependencyValidationStatus,
) -> None:
    _assert_blocked_with_reason(
        _valid_record(dependency_validation_status=status),
        f"dependency validation status is {status.value}",
    )
...
@pytest.mark.parametrize(
    ...
    [
        p
        for p in fcvr.FailClosedPosture
        if p is not fcvr.FailClosedPosture.FAIL_CLOSED_ENFORCED
    ],
)
def test_non_enforced_fail_closed_postures_fail_closed(
    posture: fcvr.FailClosedPosture,
) -> None:
    _assert_blocked_with_reason(
        _valid_record(fail_closed_posture=posture),
        f"fail-closed posture is {posture.value}",
    )
...
@pytest.mark.parametrize(
    "status",
    [
        fcvr.RuntimeGateStatus.RUNTIME_GATE_BLOCKED,
        fcvr.RuntimeGateStatus.RUNTIME_GATE_REQUIRES_MANUAL_REVIEW,
        fcvr.RuntimeGateStatus.RUNTIME_GATE_UNKNOWN,
    ],
)
def test_non_ready_runtime_gate_statuses_fail_closed(
    status: fcvr.RuntimeGateStatus,
) -> None:
    _assert_blocked_with_reason(
