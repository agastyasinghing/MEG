"""Pure supplied-input runtime operator-review handoff scaffold.

This module consumes only caller-supplied values for runtime operator-review
handoff metadata. The handoff packet is an in-memory record only. It performs
...
class OperatorReviewHandoffStatus(_ClosedValue):
    OPERATOR_REVIEW_HANDOFF_RECORDED = "operator_review_handoff_recorded"
    OPERATOR_REVIEW_HANDOFF_MISSING = "operator_review_handoff_missing"
    OPERATOR_REVIEW_HANDOFF_AMBIGUOUS = "operator_review_handoff_ambiguous"
    OPERATOR_REVIEW_HANDOFF_UNSUPPORTED = "operator_review_handoff_unsupported"
    OPERATOR_REVIEW_HANDOFF_UNKNOWN = "operator_review_handoff_unknown"


class OperatorReviewHandoffCompletenessStatus(_ClosedValue):
    OPERATOR_REVIEW_HANDOFF_COMPLETE = "operator_review_handoff_complete"
    OPERATOR_REVIEW_HANDOFF_INCOMPLETE = "operator_review_handoff_incomplete"
    OPERATOR_REVIEW_HANDOFF_AMBIGUOUS = "operator_review_handoff_ambiguous"
    OPERATOR_REVIEW_HANDOFF_UNKNOWN = "operator_review_handoff_unknown"
...
class SuppliedRuntimeOperatorReviewHandoffRecord:
    condition_id: str
    token_id: str
    outcome: str
    ...
    operator_review_handoff_status: OperatorReviewHandoffStatus
    operator_review_handoff_completeness_status: OperatorReviewHandoffCompletenessStatus
...
@dataclass(frozen=True)
class SuppliedRuntimeOperatorReviewHandoffValidationResult:
...
def supplied_runtime_operator_review_handoff_record_from_mapping(
    mapping: Mapping[str, Any],
) -> SuppliedRuntimeOperatorReviewHandoffRecord:
    """Build operator-review handoff metadata from explicitly supplied values."""

    return SuppliedRuntimeOperatorReviewHandoffRecord(
        ...
        handoff_summary=mapping["handoff_summary"],
        operator_review_summary=mapping["operator_review_summary"],
        blocked_reason_summary=mapping["blocked_reason_summary"],
        operator_review_handoff_status=_enum_value(
            OperatorReviewHandoffStatus,
            mapping["operator_review_handoff_status"],
        ),
        operator_review_handoff_completeness_status=_enum_value(
            OperatorReviewHandoffCompletenessStatus,
            mapping["operator_review_handoff_completeness_status"],
        ),
...
def validate_supplied_runtime_operator_review_handoff_record(
    record: SuppliedRuntimeOperatorReviewHandoffRecord,
) -> SuppliedRuntimeOperatorReviewHandoffValidationResult:
    """Validate a supplied operator-review handoff with fail-closed behavior."""
    ...
    trace_result = validate_supplied_runtime_trace_packet_record(
        record.supplied_runtime_trace_packet
    )
    if not trace_result.passed:
        reasons.append("supplied runtime trace packet validation failed")
    ...
    for field_name in ("condition_id", "token_id", "outcome"):
        if getattr(record, field_name) != getattr(
            ...
            reasons.append(f"{field_name} does not match supplied runtime trace packet")

    if (
        record.operator_review_summary
        != record.supplied_runtime_trace_packet.operator_review_summary
    ):
        reasons.append(
    ...
    if (
        record.operator_review_handoff_status
        is not OperatorReviewHandoffStatus.OPERATOR_REVIEW_HANDOFF_RECORDED
    ):
        reasons.append(
            "operator review handoff status is "
            f"{record.operator_review_handoff_status.value}"
        )

    if (
        record.operator_review_handoff_completeness_status
        is not OperatorReviewHandoffCompletenessStatus.OPERATOR_REVIEW_HANDOFF_COMPLETE
    ):
        reasons.append(
            "operator review handoff completeness status is "
            f"{record.operator_review_handoff_completeness_status.value}"
        )
    ...
    if reasons:
        return SuppliedRuntimeOperatorReviewHandoffValidationResult(
